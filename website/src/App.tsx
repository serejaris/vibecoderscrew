...
export default function App() {
  ...
  const usageBillingEnabled = provider.capabilities?.usageBilling === true

  // Kiro CLI monthly credit usage is an explicit action. The default query is
  // disabled so mounting the dashboard cannot call `/api/sessions/usage`; the
  // query function opts into `?refresh=1` only after the user opens the usage
  // readout, and that request returns one bounded fresh result. Month-to-date
  // total = credits_used, which the backend already sets to the TRUE total
  // (covered + overage).
  const { data: kiroUsage, refetch: refreshKiroUsage } = useQuery({
    queryKey: ['kiro-usage', provider.id],
    queryFn: () => api.sessionsUsage(true).then(d => {
      const u = d?.usage || {}
  ...
  const openKiroUsage = useCallback(() => {
    if (!usageBillingEnabled) return
    setKiroUsageOpen(true)
    void refreshKiroUsage()
  }, [refreshKiroUsage, usageBillingEnabled])
  ...
  useEffect(() => {
    if (!usageBillingEnabled || kiroUsage === 'none') setKiroUsageOpen(false)
  }, [kiroUsage, usageBillingEnabled])
  ...
  return (
      ...
      <header className="topbar topbar-glass relative flex items-center pl-3 pr-3 z-[45]" style={{ gridArea: 'topbar' }}>
        ...
        <div ref={topbarActionsRef} className="flex items-center gap-1.5 relative ml-auto">
          ...
          {(() => {
            ...
            if (usageBillingEnabled && kiroUsage !== 'none') {
              if (!kiroUsage) {
                segments.push(<button key="usage" className={`${seg} text-muted`} onClick={openKiroUsage} title={i18nT('app.kiro_credit_usage_checking')} aria-label={i18nT('app.kiro_credit_usage_checking_2')}><Coins size={12} /> {!isMobile && <Loader2 size={11} className="animate-spin" />}</button>)
              } else {
                ...
                segments.push(<button key="usage" className={kiroUsage.stale ? `${seg} opacity-60` : seg} onClick={openKiroUsage} title={title} aria-label={title}>
              ...
              <div {...(effectiveCollapsed ? { inert: '' } : {})} className={`overflow-hidden transition-all duration-200 ${effectiveCollapsed ? 'max-h-0 opacity-0' : 'max-h-16 opacity-100 mt-1'}`}>
                <div className="flex items-center border-t border-border-strong pl-3 pr-0.5 pt-2.5 pb-0.5 whitespace-nowrap">
                  ...
                  <div className="rail-community-links flex items-center gap-[5px] flex-1 min-w-0 ml-1.5 text-[12px]">
                    <a href="https://github.com/serejaris/vibecoderscrew" target="_blank" rel="noopener noreferrer" title={i18nT('app.star_kirocrew_on_github')} aria-label={i18nT('app.star_kirocrew_on_github')} className="shrink-0 rounded text-muted hover:text-text transition-colors">{i18nT('app.star_us')}</a>
                    <span aria-hidden="true" className="shrink-0 opacity-40">·</span>
                    <a href="https://github.com/serejaris/vibecoderscrew/issues" target="_blank" rel="noopener noreferrer" title={i18nT('app.report_an_issue_on_github')} aria-label={i18nT('app.report_an_issue_on_github')} className="min-w-0 overflow-hidden text-ellipsis rounded text-muted hover:text-text transition-colors">{i18nT('app.report_issue')}</a>
                  </div>
                  {/* Discord belongs to the explicit ACP/Kiro surface. Keep it
                      out of the default Codex shell so the default provider
                      never advertises Kiro account/community affordances. */}
                  {provider.id === 'acp' && (
    ...
    <Modal open={usageBillingEnabled && kiroUsageOpen} onClose={() => setKiroUsageOpen(false)} title={<span className="flex items-center gap-2"><Coins size={16} /> {i18nT('app.kiro_credits')}</span>} maxWidth={460}>
