...
describe('fmtUnit', () => {
  it('formats durations and sizes without Intl.DurationFormat', () => {
    // The implementation deliberately uses NumberFormat with `style: 'unit'`.
    // Newer Node releases expose Intl.DurationFormat, while the Node 20 /
    // Electron floor does not; the output contract must hold on both runtimes.
  ...
  it('reproduces every bare toLocale* default byte for byte, in every shipped locale', async () => {
    ...
    for (const code of ['en', 'zh-CN', 'de', 'es', 'fr', 'it', 'pt', 'ru', 'hi', 'bn']) {
      await withLanguage(code, () => {
        ...
        expect(fmtDateNumeric(INSTANT, UTC), `${code} date`)
          .toBe(INSTANT.toLocaleDateString(tag, UTC))
        expect(fmtTimeNumeric(INSTANT, UTC), `${code} time`)
          .toBe(INSTANT.toLocaleTimeString(tag, UTC))
        expect(fmtDateTimeNumeric(INSTANT, UTC), `${code} date+time`)
