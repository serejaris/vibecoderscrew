...
export default function SkillsTab() {
  ...
  // The provider context starts neutral while the persisted config loads. A
  // package-backed skill list still needs a stable, accessible section label
  // during that first render; this neutral fallback is replaced by the active
  // provider's label as soon as its config arrives.
  const packageSectionLabel = provider.labels.pluginRegistryName || 'Packages'
            ...
            {packageSkills.length > 0 && (
              <div className="mt-2">
                <div className="text-[11px] text-aim font-semibold tracking-wider px-2 py-1.5 mb-1" title={`Skills from ${provider.labels.pluginRegistryName} — read-only`}>
                  {packageSectionLabel.toUpperCase()}
                </div>
