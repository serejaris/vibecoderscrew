...
import {
  Package, Bot, Zap, Clock, ShoppingBag, Lock, Trash2, X, ArrowUp, Boxes, RefreshCw,
} from 'lucide-react'
...
export default function AppsPage() {
  ...
  // A page mount is a cache/static read.  Remote registry and manifest fetches
  // are enabled only after an explicit Refresh button or Discover tab action.
  const [registryRefreshRequested, setRegistryRefreshRequested] = useState(false)
  ...
  const { data: registryData, isLoading: registryLoading, error: registryError } = useQuery<{ apps: RegistryApp[] }>({
    queryKey: ['registry', registryRefreshRequested],
    ...
    queryFn: async () => {
      const res = await api.listRegistry({ refresh: registryRefreshRequested })
  ...
  const requestRegistryRefresh = () => setRegistryRefreshRequested(true)
  const changeTab = (next: Tab) => {
    setTab(next)
    // Selecting Discover is an explicit user action.  The default landing
    // view remains static/cache-only until the user asks to discover remotely.
    if (next === 'discover') requestRegistryRefresh()
  }
    ...
    <>
      ...
      <PageHeader
        ...
        actions={<>
          ...
          />
          <Btn
            onClick={requestRegistryRefresh}
            disabled={registryLoading && registryRefreshRequested}
            aria-label={i18nT('components.registryManager.sync_registry_apps')}
            title={i18nT('components.registryManager.sync_registry_apps')}
          >
            <RefreshCw className={`lucide-inline ${registryLoading && registryRefreshRequested ? 'animate-spin' : ''}`} />
            {i18nT('components.registryManager.sync_apps')}
          </Btn>
          <SearchInput
