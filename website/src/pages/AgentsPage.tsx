...
export default function AgentsPage({ embedded }: { embedded?: boolean } = {}) {
  ...
  const { data: usage = null } = useQuery({
    ...
    // Billing refresh is an explicit user action. Keep this historical panel
    // inert on mount so it cannot trigger the provider's /usage scrape.
    queryFn: () => api.sessionsUsage(true).then(d => (d.usage && Number.isFinite(d.usage.credits_plan)) ? d.usage : null).catch(() => null),
    enabled: false,
  })
