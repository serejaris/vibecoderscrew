...
function BindingFields({
  ...
  // Provider context is neutral for the first render while persisted config
  // loads. Keep the control labelled during that interval; the active
  // provider's wording takes over on the next render.
  const resolvedTemplateLabel = templateLabel || 'Agent Template'
  return (
    <>
      <Field label={resolvedTemplateLabel} hint={i18nT('pages.kiroCrewAgentsPage.the_agent_definition_it_boots_from_tools_mcp_ser')}>
        <SimpleSelect options={withCurrent(kiroAgentOptions, kiroAgent)} value={kiroAgent} onChange={setKiroAgent} aria-label={resolvedTemplateLabel} />
      </Field>
