...
function SuggestedPills({ setInput }: { setInput: (v: string) => void }) {
  ...
  // Keep the welcome surface entirely local on mount. Provider-backed
  // suggestions are an explicit refresh action, so a signed-out dashboard
  // never turns a passive UI render into a Kiro/AWS prompt.
  const pills = remoteSuggestions?.length ? remoteSuggestions : fallbackSuggestions

  const handleRefresh = async () => {
    setRefreshing(true)
    try {
      const fresh = await api.suggestions(true)
      if (fresh.suggestions?.length) setRemoteSuggestions(fresh.suggestions)
    } catch {}
    setRefreshing(false)
  }
