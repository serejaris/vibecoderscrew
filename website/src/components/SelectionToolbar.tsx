  ...
  useEffect(() => {
    ...
    const onMouseDown = (e: MouseEvent) => {
      ...
      if (containerRef.current && containerRef.current.contains(e.target as Node)) {
        schedule(() => { if (!window.getSelection()?.toString().trim()) setVisible(false) }, 0)
        return
      }
      setVisible(false)
    }
