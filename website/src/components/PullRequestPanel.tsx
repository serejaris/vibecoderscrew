...
export default function PullRequestPanel({
  ...
  const statusForceRefreshRef = useRef(false)
  const queryClient = useQueryClient()
  ...
  const query = useQuery<PullRequestSource>({
    ...
    // Provider-backed detail reads are explicit actions. Mounting the Changes
    // panel must remain local/cache-only; the Refresh button below is the
    // single entry point that calls this query.
    enabled: false,
    staleTime: Infinity,
    retry: false,
  ...
  const checksQuery = useQuery<{ checks: PullRequestCheck[] }>({
    ...
    refetchOnWindowFocus: false,
    refetchOnReconnect: false,
    // A pending check is shown from the last explicit provider read. Polling
    // it on a timer would turn opening a linked PR into recurring gh/glab
    // network traffic; the user can request another read with Refresh.
  })
  ...
  // failing pull request is legible without selecting it. The gateway serves
  // this request from its local chip cache; only an explicit Refresh asks the
  // server to start provider work.
  const statusUrls = useMemo(
    () => sources.slice(0, MAX_PULL_REQUEST_SOURCES).map(item => item.url),
    [sources],
  )
  useEffect(() => {
    statusForceRefreshRef.current = false
  }, [statusUrls])
  const statusQuery = useQuery<PullRequestStatusBatch>({
    queryKey: ['pull-request-statuses', statusUrls] as const,
    queryFn: async () => {
      const refresh = statusForceRefreshRef.current
      statusForceRefreshRef.current = false
      return refresh
        ? api.pullRequestStatuses(statusUrls, true)
        : api.pullRequestStatuses(statusUrls)
    },
  ...
  const handleRefresh = () => {
    ...
    // Explicitly refresh both the selected detail payload and the strip's
    // lightweight status batch. The backend only starts provider work for the
    // status request when this flag is true.
    statusForceRefreshRef.current = true
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      {query.isFetching && <div className="flex-1 flex items-center justify-center gap-2 text-[13px] text-muted"><Loader className="lucide-inline animate-spin" />{i18nT('components.pullRequestPanel.loading_source_provider')}</div>}
      {query.error && (
      ...
      {!query.isFetching && !query.error && !source && selected && (
        <div className="flex-1 flex items-center justify-center px-6">
          <Btn
            type="button"
            onClick={handleRefresh}
            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border border-border bg-transparent text-[12px] text-muted hover:text-text hover:bg-bg-hover cursor-pointer"
            aria-label={i18nT('components.pullRequestPanel.refresh_pull_request')}
            title={i18nT('components.pullRequestPanel.refresh_pull_request')}
          >
            <RefreshCw className="lucide-inline" />
            {i18nT('components.pullRequestPanel.refresh_pull_request')}
          </Btn>
        </div>
      )}
