...
export interface KiroPrerequisiteOperation {
  ...
  url: string
  error: string
  /** Stable backend error code for provider-specific setup actions. */
  code?: string
}

export interface KiroPrerequisiteStatus {
  /** Selected runtime provider. Older gateways omit this field. */
  provider?: 'acp' | 'codex'
  platform: string
  installed: boolean
...
export const api = {
  ...
  /** Explicitly launch the local ``codex login`` flow after a user click. */
  loginCodex: () =>
    post('/api/codex/login').then(j) as Promise<KiroPrerequisiteStatus>,
  ...
  sessionsUsage: (refresh = false) => fetch('/api/sessions/usage' + (refresh ? '?refresh=1' : '')).then(j),
  ...
  models: (refresh = false) => fetch('/api/models' + (refresh ? '?refresh=1' : '')).then(j),
  ...
  // Cache-only by default; callers set refresh=true for an explicit provider
  // read (the dashboard never starts gh/glab work from a timer).
  pullRequestStatuses: (urls: string[], refresh = false) => post(
    '/api/source/pull-request/status',
    refresh ? { urls, refresh: true } : { urls },
  ).then(j) as Promise<PullRequestStatusBatch>,
  ...
  listRegistry: (opts?: { refresh?: boolean }) => fetch(`/api/apps/registry${opts?.refresh ? '?refresh=1' : ''}`).then(j) as Promise<{ apps: any[]; serverPlatform: { os: string; arch: string } }>,
