...
function renderPanel(onAddToChat = vi.fn(), { load = true }: { load?: boolean } = {}) {
  ...
  // Detail and status reads are deliberately explicit in production. Legacy
  // fixture callers opt into that same user action so their assertions remain
  // focused on the loaded panel; mount-only regression tests pass load:false.
  if (load) fireEvent.click(screen.getByRole('button', { name: 'Refresh pull request' }))
  return {
    client,
    onAddToChat,
    ...rendered,
...
describe('PullRequestPanel', () => {
  it('mounts from local status cache without provider reads or timers', async () => {
    renderPanel(vi.fn(), { load: false })

    await waitFor(() =>
      expect(mockApi.pullRequestStatuses).toHaveBeenCalledWith([github.url, gitlab.url]),
    )
    expect(mockApi.pullRequestSource).not.toHaveBeenCalled()
    expect(mockApi.pullRequestChecks).not.toHaveBeenCalled()
    ...
    try {
      ...
      expect(mockApi.pullRequestSource).not.toHaveBeenCalled()
      expect(mockApi.pullRequestChecks).not.toHaveBeenCalled()
      expect(mockApi.pullRequestStatuses).toHaveBeenCalledTimes(1)
    } finally {
      vi.useRealTimers()
    }
  })
  ...
  it('separates network refresh from actual running checks', async () => {
    ...
    expect(mockApi.pullRequestStatuses).toHaveBeenLastCalledWith([github.url, gitlab.url], true)
  ...
  it('does not poll pending checks after the explicit refresh', async () => {
    vi.useFakeTimers()
    const pendingCheck = {
      ...github.checks[0],
      status: 'IN_PROGRESS',
    ...
    try {
      ...
      expect(mockApi.pullRequestSource).toHaveBeenCalledTimes(1)
      expect(mockApi.pullRequestChecks).toHaveBeenCalledTimes(1)
      expect(client.getQueryData<PullRequestSource>(['pull-request-source', github.url])?.checks[0].bucket)
        .toBe('pending')
    } finally {
      unmount()
      vi.useRealTimers()
  ...
  it('does not retry a failed check read on a timer', async () => {
    vi.useFakeTimers()
    const pendingCheck = {
      ...github.checks[0],
      status: 'IN_PROGRESS',
  ...
  it('fetches the selected GitLab merge request', async () => {
    ...
    await waitFor(() => expect(mockApi.pullRequestSource).toHaveBeenCalledWith(gitlab.url, true))
