...
describe('InstancesViewport', () => {
  ...
  it('does not connect or warm persisted connected instances on mount', async () => {
    // The list response may carry a persisted connected status, but the
    // viewport is read-only until the owner selects the tab / presses Retry.
  ...
  it('does not warm an instance whose tunnel is down', async () => {
    ...
    const { store } = renderWithProviders(<InstancesViewport />)
    await waitFor(() => expect(api.listInstances).toHaveBeenCalled())
    // A down instance is never warmed by the viewport; it stays a sticky tab to be clicked.
    expect(api.connectInstance).not.toHaveBeenCalled()
    expect(store.getState().instances.warm['cd-1']).toBeUndefined()
    expect(document.querySelector('iframe')).toBeNull()
  })
  ...
  it('treats mc-auth-expired as disconnected until explicit Retry', async () => {
    vi.mocked(api.listInstances).mockResolvedValue({
      instances: [
        {
          id: 'cd-1',
    ...
    window.dispatchEvent(
      new MessageEvent('message', {
        data: { type: 'mc-auth-expired', v: 1 },
        origin: `${window.location.protocol}//${window.location.hostname}:7778`,
      }),
    )
    expect(await screen.findByText(/needs authentication/i)).toBeInTheDocument()
    expect(api.refreshInstanceToken).not.toHaveBeenCalled()
