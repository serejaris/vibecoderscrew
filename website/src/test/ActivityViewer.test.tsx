...
describe('ActivityViewer', () => {
  ...
  it('renders each detected PR as a source selector and waits for an explicit refresh', () => {
    render(
      <ActivityViewer
        {...baseProps}
        view="changes"
    ...
    // Opening the Changes view is cache-only. Provider-backed detail reads are
    // deliberately opt-in through the refresh action, so mounting the panel
    // never starts a gh/glab request or leaves a perpetual loading state.
    expect(screen.getByRole('button', { name: 'Refresh pull request' })).toBeInTheDocument()
    expect(api.pullRequestSource).not.toHaveBeenCalled()
  })
