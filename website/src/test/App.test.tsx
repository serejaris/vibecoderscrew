...
const mockProvider = vi.hoisted(() => ({
  current: { id: 'acp', capabilities: { usageBilling: true } },
}))
vi.mock('../providers/context', () => ({ useProvider: () => mockProvider.current }))
...
afterEach(() => {
  mockProvider.current = { id: 'acp', capabilities: { usageBilling: true } }
})
...
describe('App routing', () => {
  ...
  describe('first-run Privacy chapter', () => {
    ...
    it('opens after Import setup and gates the Customize chapter', async () => {
      ...
      expect(within(dialog).getAllByText('Telemetry is off').length).toBeGreaterThan(0)
  ...
  it('renders nav items', () => {
    ...
    expect(screen.getByLabelText('Star VibecodersCrew on GitHub')).toBeInTheDocument()
  ...
  it('renders Kiro Crew branding', () => {
    ...
    // The public fork's canonical wordmark is VibecodersCrew.
    expect(screen.getAllByText('VibecodersCrew').length).toBeGreaterThan(0)
    localStorage.removeItem('mc-nav')
  })
  ...
  it('hosts the collapse control in the nav menu row and hides the Main group heading', () => {
    ...
    expect(within(nav).getByText('VibecodersCrew')).toBeInTheDocument()
...
async function openUsageReadout() {
...
describe('Kiro credits pill', () => {
  ...
  it('does not fetch usage on mount and shows an inert checking state', async () => {
    const { api } = await import('../api/client')
    renderWithProviders(<App />, { route: '/chat' })
    expect(await screen.findByTitle(/Kiro credit usage/)).toBeInTheDocument()
    expect(api.sessionsUsage).not.toHaveBeenCalled()
  })

  it('refreshes usage only after the user opens the readout', async () => {
    const { api } = await import('../api/client')
    vi.mocked(api.sessionsUsage).mockResolvedValueOnce({ usage: {} } as never)
    renderWithProviders(<App />, { route: '/chat' })
    await openUsageReadout()
    await waitFor(() => expect(api.sessionsUsage).toHaveBeenCalledWith(true))
  })
...
describe('provider usage capability gate', () => {
  it('does not render the usage pill or call sessionsUsage for Codex', async () => {
    const { api } = await import('../api/client')
    mockProvider.current = { id: 'codex', capabilities: { usageBilling: false } }
    vi.mocked(api.sessionsUsage).mockClear()
    ...
    expect(screen.queryByLabelText('Kiro Discord community')).not.toBeInTheDocument()
    expect(screen.queryByText(/Manage account/)).not.toBeInTheDocument()
    expect(api.sessionsUsage).not.toHaveBeenCalled()
  })

  it('fails closed when a provider omits usageBilling', async () => {
    const { api } = await import('../api/client')
    mockProvider.current = { id: 'codex', capabilities: {} }
    vi.mocked(api.sessionsUsage).mockClear()
...
describe('Kiro credits pill — edge cases', () => {
  ...
  it('keeps the passive pill until an explicit refresh reports an unavailable provider', async () => {
    const { api } = await import('../api/client')
