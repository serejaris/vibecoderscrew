...
describe('CI supplies a base commit on both paths', () => {
  ...
  const ci = existsSync(ciPath) ? readFileSync(ciPath, 'utf-8') : null

  if (!ci) {
    // The public fork is source-only and intentionally does not ship a GitHub
    // Actions workflow. The pure verdict tests above still cover the gate; this
    // wiring contract is meaningful only when a workflow exists to inspect.
    it('accepts the source-only tree without a CI workflow', () => {
      expect(ci).toBeNull()
    })
  } else {
