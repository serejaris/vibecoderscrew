 ...
 *
 * The app-specific concern vs. a plain Electron app is unchanged: the
...
// Source-only builds have no distribution host. A release operator must pass an
// explicit feed base (or KIROCREW_UPDATE_FEED) before opting into updates.
...
// from it. The caller owns the pointer/bytes host contract for that feed.
const {
...
// Manual download links are opt-in too. The host is supplied by the embedding
// app; keeping this empty prevents a source checkout from advertising a
// distribution endpoint it does not own.
const DOWNLOAD_BASE = "";
 ...
 *   (switching from a newer insider build to a stable build), which is why allowDowngrade
...
function buildFeedBase({ base, channel }) {
  const b = String(base || DEFAULT_FEED_BASE).replace(/\/+$/, "");
  if (!b) {
    throw new Error("feed base is required; configure an explicit update source");
  }
 ...
 * in the VibecodersCrew home directory, never inside the app bundle.
 *
 ...
 * @param {string} [downloadBase] explicit artifact host/path
 * @returns {string|null}
 */
function manualDownloadUrl(channel, osPlatform, downloadBase = DOWNLOAD_BASE) {
  if (!KNOWN_CHANNELS.has(channel)) return null;
  if (!downloadBase) return null;
  ...
  const file = osPlatform === "darwin"
    ? "VibecodersCrew.dmg"
    : osPlatform === "linux"
      ? "VibecodersCrew-x86_64.AppImage"
      : null;
  if (!file) return null;
  return `${String(downloadBase).replace(/\/+$/, "")}/desktop/${channel}/latest/${file}`;
}
...
function initAutoUpdate(deps) {
  const {
    ...
    downloadBase = process.env.KIROCREW_UPDATE_DOWNLOAD || DOWNLOAD_BASE,
    // Source-only builds stay inert unless the embedding distribution opts in
    // explicitly. This default protects direct callers that omit the flag.
  ...
  function getInfo() {
    ...
    return {
      ...
      downloadUrl: updatesEnabled
        ? manualDownloadUrl(currentChannel(), osPlatform, downloadBase)
        : null,
    };
  }
  ...
  async function promptInstall(versionName, notes) {
    const { response } = await dialog.showMessageBox({
      ...
      defaultId: 0,
      cancelId: 1,
      title: "VibecodersCrew update ready",
      message: `VibecodersCrew ${versionName || ""} is ready to install.`.trim(),
      detail:
        (notes || "").slice(0, 500) +
        "\n\nVibecodersCrew will stop the local gateway, install the update, and relaunch.",
    });
    if (response === 0) {
      ...
      try {
        new Notification({
          ...
          body: "VibecodersCrew will finish updating the next time you quit.",
        }).show();
