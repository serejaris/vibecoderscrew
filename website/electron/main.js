...
// Initialise before resolvePort()/config migration can emit diagnostics. The
// logger's path resolver is lazy, so app.getPath("logs") is only touched when
// the first record is written and its fallback remains safe before ready.
const gatewayLogger = createGatewayLogger({ getLogPath: gatewayLogPath });
...
function resolvePort() {
  ...
  if (raw) {
    ...
    if (isNaN(n) || n < 1 || n > 65535) {
      gatewayLogger.error(`Invalid KIROCREW_PORT="${raw}", falling back to 5476`);
      return 5476;
    }
    return n;
  }
  ...
  gatewayLogger.log("No usable dashboard.url port in the data home, falling back to 5476");
  return 5476;
}
...
if (migrateRemoteHostConfig(store, PORT)) {
  gatewayLogger.log(`Migrated legacy remoteHost to remoteHosts[${PORT}]`);
}
...
app.name = identityFamily(app.getVersion()) === "nightly"
  ? "VibecodersCrew Nightly" : "VibecodersCrew";
...
// pinning targets Electron rather than VibecodersCrew). Match the packaged appId
// (build.appId = "dev.serejaris.vibecoderscrew"); nightly gets a distinct id so it
...
if (IS_WIN) {
  const appUserModelId = identityFamily(app.getVersion()) === "nightly"
    ? "dev.serejaris.vibecoderscrew.nightly" : "dev.serejaris.vibecoderscrew";
...
async function resolveGatewayConflict() {
  ...
  const { response } = await dialog.showMessageBox({
    ...
    message: `${other.displayName} (${decision.otherVersion}) is already running with your Vibecoders Crew data.`,
    detail: canTakeover
      ? `Only one Vibecoders Crew app can use ~/.kiro/crew at a time. Quit ${other.displayName} and continue here?`
      : `Only one Vibecoders Crew app can use ~/.kiro/crew at a time. Quit ${other.displayName}, then reopen this app.`,
...
async function offerRelocationIfUnupdatable() {
  ...
  try {
    ({ response } = await dialog.showMessageBox({
      type: "warning",
      title: "Move Vibecoders Crew to Applications?",
  ...
  try {
    await dialog.showMessageBox({
      type: "error",
      message: "Could not move Vibecoders Crew automatically.",
      detail: "Drag Vibecoders Crew into your Applications folder, then reopen it from there.",
      buttons: ["OK"],
    });
...
// Resolve the VibecodersCrew project root (the tree that ships `agents/` + `skills/`)
...
function spawnGateway(resolve) {
        ...
        child.on("exit", (code, signal) => {
          ...
          if (signal === "SIGKILL") {
            glog("HINT: SIGKILL on a freshly-spawned bundled binary almost always means macOS Gatekeeper blocked an unsigned/quarantined nested executable. On the recipient's Mac run: xattr -cr <path to VibecodersCrew.app>");
          }
...
function stopGateway() {
  stopGatewayGracefully().catch((err) => gatewayLogger.error(`Gateway stop failed: ${err?.message || err}`));
}
...
function fetchRemoteToken(port) {
  ...
  if (validationErr) {
    gatewayLogger.error(`Refusing SSH token fetch: ${validationErr}`);
  ...
  return new Promise((resolve) => {
    ...
    glog(`SSH token fetch: ssh ${host} for port ${effectivePort}`);
    execFile("/usr/bin/ssh", sshArgs, { timeout: Math.max(store.get("sshTimeoutMs") || 20000, 5000) }, (err, stdout, stderr) => {
      if (err) {
        gatewayLogger.error(`SSH token fetch failed: ${err.message}`);
        if (stderr) gatewayLogger.error(`SSH stderr: ${stderr.trim().slice(0, 500)}`);
...
/** Read CSS custom properties from the active VibecodersCrew dashboard. */
...
function setupWindowContents(win, backendUrl) {
  ...
  function applyTitle() {
    ...
    if (!IS_WIN) {
      ...
      win.setTitle(`Vibecoders Crew ${suffix}`);
      return;
    }
    // Windows: the bare "[:5476]" read as part of the product name and confused
    // users, so the primary local window is just "Vibecoders Crew"; keep the suffix
    ...
    win.setTitle(suffix ? `Vibecoders Crew ${suffix}` : "Vibecoders Crew");
  }
  ...
  function browserPanel(panelId, { create = true } = {}) {
    ...
    entry.control = createControlPlane({
      ...
      onAudit: (event, detail) => {
        glog(`[browser-control] ${id} ${event} ${JSON.stringify(detail)}`);
      },
    });
    entry.gate = () =>
      canAgentControl({
  ...
  win._mcAgentChannel = createAgentCommandChannel({
    ...
    onError: (err, context) =>
      glog(`[browser-agent-channel] ${context}: ${err && err.message}`),
  });
...
function createWindow() {
  ...
  mainWindow.webContents.on("did-navigate", (_e, _url, httpCode) => {
    onNavigate(httpCode).catch((err) => gatewayLogger.error(`Token retry failed: ${err}`));
  });
...
async function promptRemoteHost() {
  ...
  const html = `<!DOCTYPE html><html><head><style>
    ...
    <label>vibecoderscrew binary path</label>
  ...
  promptWin.on("closed", () => {
    ...
    } catch (e) { gatewayLogger.error(`Failed to parse remote host settings: ${e.message}`); }
  });
}
...
/**
 * Best-effort force-stop of whatever holds `port`, scoped to VibecodersCrew processes
...
async function showUnrecoverableGatewayError(win, port) {
  ...
  const action = await showGatewayErrorDialog(win, {
    title: `Vibecoders Crew — backend stuck on port ${port}`,
    message: `The Vibecoders Crew backend is wedged and cannot be stopped — it is in an `
...
async function showLoadingThenConnect(win, backendUrl = BACKEND_URL) {
  ...
  try {
    ...
    // force-stop the stuck VibecodersCrew process. Only meaningful for OUR own port.
    const portConflict = failedToStart && backendUrl === BACKEND_URL && isPortInUse(logTail);

    let title, message;
    if (portConflict) {
      title = `Vibecoders Crew — port ${PORT} already in use`;
      message = `Another Vibecoders Crew gateway is already using port ${PORT} (it may be wedged). `
        + `Force-stop it and retry, or quit. From a terminal you can also run: `
        + `vibecoderscrew stop --port ${PORT}`;
    } else if (failedToStart) {
      title = "Vibecoders Crew — gateway failed to start";
      message = err.message;
    } else {
      title = "Vibecoders Crew — can't reach the gateway";
      message = "Could not connect to the Vibecoders Crew backend. Make sure "
        + "'vibecoderscrew gateway' is running, or check vibecoderscrew doctor.";
    }
...
async function openNewConnectionWindow() {
  ...
  const html = `<!DOCTYPE html><html><head><style>
    ...
    <div class="hint">Connect to a Vibecoders Crew gateway running on another port</div>
...
// VibecodersCrew.app, not the terminal that launched a dev build.
function showScreenPermissionDialog() {
  ...
  dialog
    .showMessageBox({
      ...
      message: "Allow Vibecoders Crew to capture the screen",
      detail:
        "The screen-snip tool needs macOS Screen Recording permission. Open System Settings › Privacy & Security › Screen Recording, enable Vibecoders Crew, then try the snip again.",
  ...
  dialog
    .showMessageBox({
      ...
      message: restricted
        ...
        : "Allow Vibecoders Crew to use the microphone",
      detail: restricted
        ? "Voice input needs macOS Microphone permission, but access is restricted by a device-management policy on this Mac. Contact whoever manages it to allow microphone access for Vibecoders Crew."
        : "Voice input needs macOS Microphone permission, and macOS will not ask again once it has been denied. Open System Settings › Privacy & Security › Microphone, enable Vibecoders Crew, then try the mic again.",
