...
const { manualDownloadUrl } = require("../auto-update");
const TEST_DOWNLOAD_BASE = "https://cdn.example.dev";

test("manualDownloadUrl: per-platform artifact on the byte host", () => {
  assert.strictEqual(
    manualDownloadUrl("nightly", "darwin", TEST_DOWNLOAD_BASE),
    `${TEST_DOWNLOAD_BASE}/desktop/nightly/latest/VibecodersCrew.dmg`,
  );
  assert.strictEqual(
    manualDownloadUrl("stable", "linux", TEST_DOWNLOAD_BASE),
    `${TEST_DOWNLOAD_BASE}/desktop/stable/latest/VibecodersCrew-x86_64.AppImage`,
  );
  assert.strictEqual(
    manualDownloadUrl("insider", "darwin", TEST_DOWNLOAD_BASE),
    `${TEST_DOWNLOAD_BASE}/desktop/insider/latest/VibecodersCrew.dmg`,
  );
});
...
test("manualDownloadUrl: null wherever there is no publish lane", () => {
  ...
  assert.strictEqual(manualDownloadUrl("dev", "darwin", TEST_DOWNLOAD_BASE), null);
  assert.strictEqual(manualDownloadUrl("", "darwin", TEST_DOWNLOAD_BASE), null);
  assert.strictEqual(manualDownloadUrl("nightly", "win32", TEST_DOWNLOAD_BASE), null);
  assert.strictEqual(manualDownloadUrl(undefined, undefined, TEST_DOWNLOAD_BASE), null);
});

test("manualDownloadUrl: no explicit artifact source means no link", () => {
  assert.strictEqual(manualDownloadUrl("nightly", "darwin"), null);
});
