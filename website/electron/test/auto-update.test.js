...
test("buildFeedBase requires an explicit source in the source-only release", () => {
  assert.strictEqual(DEFAULT_FEED_BASE, "");
  assert.throws(
    () => buildFeedBase({ channel: "nightly" }),
    /feed base is required/,
  );
});
...
// artifact urls via newUrlFromBase(fileUrl, base). A caller-supplied
// pointer/bytes host split relies on the UNDOCUMENTED-but-structural behaviour
// that an ABSOLUTE file url ignores the base. A library upgrade that changes
// this must fail CI here, not strand installs in the field.
// ---------------------------------------------------------------------------
...
test("CONTRACT: absolute artifact urls pass through newUrlFromBase unchanged (pointer/bytes split)", () => {
  ...
  const base = newBaseUrl(buildFeedBase({ base: "https://pointer.example.dev/feed", channel: "nightly" }));
  const absolute = "https://artifact.example.dev/desktop/nightly/0.1.0-nightly.20260728t112233/VibecodersCrew-arm64.dmg";
...
test("CONTRACT: relative channel-file names resolve under the feed base directory", () => {
  ...
  assert.strictEqual(
    ...
    "https://pointer.example.dev/feed/nightly/latest-mac.yml",
  );
});
...
function makeDeps(opts = {}) {
  const {
    ...
    resourcesPath = "/Applications/VibecodersCrew.app/Contents/Resources",
    bundleWritable = true,
    updatesEnabled = true,
  } = opts;
...
test("auto-update defaults to a no-op when the caller omits explicit opt-in", async () => {
  const { deps, calls } = makeDeps();
  delete deps.updatesEnabled;
  const updater = initAutoUpdate(deps);
  assert.strictEqual(updater.disabled, "community-fork");
  await updater.check();
  assert.deepStrictEqual(calls.setFeedURL, []);
...
test("translocated bundle returns disabled:'translocated' and never arms the updater", () => {
  const { deps, calls } = makeDeps({
    resourcesPath: "/private/var/folders/ab/cd/d/AppTranslocation/UUID/d/VibecodersCrew.app/Contents/Resources",
  });
  const u = initAutoUpdate(deps);
...
test("read-only volume returns disabled:'volume' and never arms the updater", () => {
  const { deps, calls } = makeDeps({
    resourcesPath: "/Volumes/VibecodersCrew 1.0.0/VibecodersCrew.app/Contents/Resources",
...
test("WRITABLE volume still arms: an external disk is not a read-only image", () => {
  ...
  const { deps, calls } = makeDeps({
    resourcesPath: "/Volumes/External SSD/VibecodersCrew.app/Contents/Resources",
    bundleWritable: true,
  });
  const u = initAutoUpdate(deps);
  assert.strictEqual(u.disabled, undefined);
...
test("guard is macOS-only: a linux /Volumes-shaped path still arms", () => {
  ...
  const { deps, calls } = makeDeps({
    osPlatform: "linux",
    resourcesPath: "/Volumes/whatever/VibecodersCrew.app/Contents/Resources",
    bundleWritable: false,
  });
  const u = initAutoUpdate(deps);
  assert.strictEqual(u.disabled, undefined);
