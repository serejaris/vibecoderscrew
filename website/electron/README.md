...
# VibecodersCrew Desktop (Electron)

Desktop shell for the Vibecoders Crew web dashboard on macOS and Linux. It
automatically starts `vibecoderscrew gateway` and connects to `localhost:5476`.
...
2. Launch `vibecoderscrew gateway` when needed
...
sudo rm -rf /Applications/VibecodersCrew.app
sudo cp -R "$APP_DIR/VibecodersCrew.app" /Applications/VibecodersCrew.app
```

Launch via Spotlight (Cmd+Space → "Vibecoders Crew"), Dock, or
`open /Applications/VibecodersCrew.app`.
...
Output goes to `electron/dist/`. This is an operator-local development build;
desktop bundles are not attached to or advertised by source-only releases.
...
launchctl stop dev.vibecoderscrew.gateway
launchctl start dev.vibecoderscrew.gateway
```
...
launchctl unload ~/Library/LaunchAgents/dev.vibecoderscrew.gateway.plist 2>/dev/null
rm -f ~/Library/LaunchAgents/dev.vibecoderscrew.gateway.plist
```
...
2. `vibecoderscrew` installed on the remote host. The technical `kirocrew`
   compatibility alias is accepted too; the default auto-discovers across
   common install layouts — no configuration needed unless you installed
   somewhere unusual.
...
3. Leave the binary path at the default unless you installed VibecodersCrew
   somewhere unusual. The default tries, in order:
...
5. Optionally set a **Remote PATH** if VibecodersCrew needs additional directories
...
The app will SSH into the configured remote host and run `vibecoderscrew token` on
...
`~/Library/Application Support/VibecodersCrew/config.json`:
...
| "kirocrew binary not found in any of …" | Install VibecodersCrew from this repository (or keep the `kirocrew` compatibility alias), or set a custom path |
