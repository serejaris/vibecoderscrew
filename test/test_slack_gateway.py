                                                ...
                                                with patch(
                                                    ...
                                                    with patch(
                                                        ...
                                                        return_value=MagicMock(
                                                            returncode=0, stdout="kiro-cli 1.30.0"
                                                        ),
                                                    ):
                                                        orch._init_services()
...
class TestCheckForUpdates:
    ...
    async def test_no_update_available(self):
        ...
        with patch("kiro_crew.dashboard.handlers._do_update_check", new_callable=AsyncMock):
            with patch("kiro_crew.dashboard.handlers._update_info", {"available": False}):
...
class TestBgSessionDashboardBranch:
    ...
    async def test_bg_session_prints_dashboard_url(self):
        ...
        # Gateway boot must not create an ACP/provider pool.  The first
        # provider process is reserved for an explicit user turn.
        orch.sessions.start_pool.assert_not_awaited()


class TestCheckMissingDepsReadOnly:
    """Dependency readiness reports an explicit remedy without installing."""

    def test_missing_deps_does_not_run_pip(self, capsys):
        orch = _make_orchestrator()
        with patch("importlib.util.find_spec", return_value=None):
            with patch("subprocess.run") as mock_run:
                missing = orch._check_missing_deps()

        assert missing == ["snowballstemmer>=1.0", "PyYAML>=6,<7"]
        mock_run.assert_not_called()
        assert "pip install" in capsys.readouterr().out

    def test_no_missing_deps_is_quiet_and_read_only(self):
        orch = _make_orchestrator()
        with patch("importlib.util.find_spec", return_value=MagicMock()):
            with patch("subprocess.run") as mock_run:
                assert orch._check_missing_deps() == []
        mock_run.assert_not_called()
