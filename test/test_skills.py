...
class TestLazyLoadContext:
    ...
    def test_ranking_prefers_newest_skills_without_usage_ledger(self, tmp_path):
        loader = self._make(tmp_path, n_on_demand=4)
        # The source-only loader has no persistent usage ledger. Touch od3 after
        # the other fixtures so recency provides a deterministic ranking signal.
        assert loader._usage is None
        newest = tmp_path / "skills" / "od3" / "SKILL.md"
        newest.touch()
        # Ranking applies only on the opt-in (integer-budget) path; a large
        # budget shows all skills AND exercises the recency ordering.
...
class TestSearchSkills:
    ...
    def test_search_does_not_record_usage(self, tmp_path):
        ...
        assert loader._usage is None  # source-only builds persist no activity log
