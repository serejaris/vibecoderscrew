...
class TestLoadExternalRegistries:
    ...
    @pytest.mark.asyncio
    async def test_cache_miss_does_not_fetch_without_explicit_refresh(self, cache_dir, monkeypatch):
        mock_reg = MagicMock()
        mock_reg.name = "offline"
        ...
        fetch = AsyncMock(side_effect=AssertionError("passive browse cloned a registry"))
        monkeypatch.setattr("kiro_crew.apps.registry._fetch_and_cache_external_registry", fetch)

        assert await _load_external_registries() == []
        fetch.assert_not_awaited()

    @pytest.mark.asyncio
    async def test_refresh_marker_allows_remote_fetch(self, cache_dir, monkeypatch):
        mock_reg = MagicMock()
        mock_reg.name = "offline"
        mock_reg.repo = "https://github.com/acme/apps"
        mock_reg.branch = "main"
        ...
        assert await _load_external_registries(allow_remote=True) == []
        fetch.assert_awaited_once_with(mock_reg)
