...
def test_ensure_cli_shims_on_path_creates_primary_and_compatibility_alias(tmp_path, monkeypatch):
    """Setup publishes the canonical command and keeps the old alias usable."""
    exe = _fake_frozen_exe(tmp_path)
    _simulate_frozen_app(monkeypatch, exe)
    bin_dir = tmp_path / "localbin"

    changed = agent.ensure_cli_shims_on_path(bin_dir=bin_dir)

    assert set(changed) == {str(bin_dir / "vibecoderscrew"), str(bin_dir / "kirocrew")}
    for name in ("vibecoderscrew", "kirocrew"):
        link = bin_dir / name
        assert link.is_symlink()
        assert os.path.realpath(link) == os.path.realpath(exe)
        assert os.access(link, os.X_OK)
    assert agent.ensure_cli_shims_on_path(bin_dir=bin_dir) == ()
