...
def _lines(name: str) -> list[str]:
    if not WORKFLOWS.is_dir():
        ...
        pytest.skip("source-only fork ships no GitHub Actions workflows")
...
class TestReusableWorkflowPermissions:
    ...
    def test_build_desktop_has_no_windows_leg(self) -> None:
        ...
        """
        if not (WORKFLOWS / "build-desktop.yml").is_file():
            import pytest

            pytest.skip("source-only fork ships no desktop build workflow")
        ...
        workflow = yaml.safe_load((WORKFLOWS / "build-desktop.yml").read_text(encoding="utf-8"))
        runners = {
            entry["os"]
