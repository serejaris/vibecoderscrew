...
def _jobs(path: Path) -> dict:
    if not path.is_file():
        ...
        pytest.skip("source-only fork ships no desktop update-feed workflow")
    return yaml.safe_load(path.read_text(encoding="utf-8"))["jobs"]


def _require_update_feed_workflows() -> None:
    if not MAC_WORKFLOW.is_file() or not LINUX_WORKFLOW.is_file():
        import pytest

        pytest.skip("source-only fork ships no desktop update-feed workflows")
...
def test_mac_lane_keeps_the_legacy_json_feed_as_a_transition_bridge() -> None:
    ...
    """
    _require_update_feed_workflows()
...
def test_feed_chain_steps_share_one_skip_gate() -> None:
    ...
    for path, job, names in (
        ...
        assert all(
            g == "env.HAS_SIGNING_SECRETS" for g in gates.values()
...
def test_linux_missing_appimage_fails_loudly() -> None:
    ...
    assert (
        "Expected exactly one AppImage" in run
...
def test_publisher_documents_the_expected_channel_set() -> None:
    ...
    if not CLI_WORKFLOW.is_file():
        import pytest

        pytest.skip("source-only fork ships no CLI publishing workflow")
