...
class TestInstallId:
    ...
    def test_create_false_never_returns_process_local_fallback(self, _isolated_home, monkeypatch):
        def denied():
...
class TestFailOpen:
    ...
    def test_gateway_has_no_beacon_wiring(self):
        """The source-only gateway does not start an outbound beacon thread.

        Beacon delivery is intentionally absent from the standalone source fork;
        keeping the assertion on the production entry point prevents a future
        refactor from silently restoring external telemetry during boot.
        """
        import inspect
...
class TestConfigDefaults:
    ...
    def test_community_edition_hard_disables_outbound_telemetry(self, _isolated_home, monkeypatch):
        from kiro_crew.config.loader import TelemetryConfig
