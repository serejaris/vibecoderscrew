...
BENIGN_SPAWNS: frozenset[str] = frozenset(
    {
        ...
        # Explicit owner action: resolved Codex binary plus the literal
        # `login` subcommand. The command, argv, and inherited cwd are not
        # agent-selected; the service also applies the same channel-credential
        # env scrub as the Codex App Server spawn. It stays allow-listed rather
        # than sandbox-wrapped because Codex must open the operator's browser
        # and read/write its own ChatGPT credential cache.
        "providers/codex.py::_run",
