...
def test_provider_executable_candidates_ignore_path_in_strict_mode(monkeypatch, tmp_path) -> None:
    ...
    assert (
        source.provider_executable_candidates("gh") == source._PROVIDER_EXECUTABLE_CANDIDATES["gh"]
    )
...
def test_provider_executable_strict_mode_rejects_user_owned_install(monkeypatch, tmp_path) -> None:
    executable = tmp_path / "gh"
    executable.write_text("#!/bin/sh\nexit 0\n")
...
def test_provider_executable_rejects_binary_owned_by_another_user(monkeypatch, tmp_path) -> None:
    executable = tmp_path / "gh"
    executable.write_text("#!/bin/sh\nexit 0\n")
    executable.chmod(0o755)
...
def test_provider_executable_strict_mode_rejects_untrusted_ancestor(monkeypatch, tmp_path) -> None:
...
def test_merge_state_settled_considers_both_fields(pair: tuple[str, str], settled: bool) -> None:
...
def test_full_payload_and_chip_projections_agree_on_the_merge_pair() -> None:
    ...
    assert {key: value for key, value in projected.items() if key.startswith("merge")} == chip
...
async def test_status_endpoint_warms_allowlist_before_parsing_self_hosted_urls(
    ...
    # The authorized self-managed URL survived validation, but a cache-only
    # read never schedules provider work.
    refresh.assert_not_called()

    async with TestClient(TestServer(_app())) as client:
        response = await client.post(
            "/api/source/pull-request/status", json={"urls": [url], "refresh": True}
        )
        assert response.status == 200

    # An explicit refresh reaches the scheduler with the validated URL.
    refresh.assert_called_once_with([url], force=True)
...
@pytest.mark.asyncio
async def test_status_endpoint_serves_cache_and_refreshes_only_on_explicit_action(
    monkeypatch,
) -> None:
    """The strip reads cache by default; ``refresh: true`` starts provider work."""
    ...
    async with TestClient(TestServer(app)) as client:
        response = await client.post(
            "/api/source/pull-request/status",
            json={"urls": [known, unknown], "refresh": True},
        )
        assert response.status == 200
...
def test_status_from_full_payload_projects_lifecycle_and_ci() -> None:
    ...
    assert source.status_from_full_payload({"state": "closed", "draft": True}) == {
        "state": "closed"
    }
...
async def test_full_fetch_writes_through_to_chip_cache(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "_fetch_github",
        AsyncMock(
            return_value={"state": "MERGED", "draft": False, "checks": [{"bucket": "passed"}]}
        ),
    )
...
def test_record_full_payload_clears_flap_tracker_on_change() -> None:
    ...
    try:
        ...
        source.record_full_payload_status(
            url, {"state": "MERGED", "checks": [{"bucket": "passed"}]}
        )
...
async def test_refresh_check_status_issues_pending_follow_up_force(monkeypatch) -> None:
    ...
    monkeypatch.setattr(source, "_fetch_check_status", AsyncMock(return_value={"state": "open"}))
...
async def test_chip_refresh_damps_projection_flap(monkeypatch) -> None:
    ...
    # changed transition.
    monkeypatch.setattr(source, "_fetch_check_status", AsyncMock(return_value={"state": "draft"}))
...
async def test_status_endpoint_bounds_urls_and_rejects_non_list_bodies(monkeypatch) -> None:
    ...
    async with TestClient(TestServer(app)) as client:
        accepted = await client.post(
            "/api/source/pull-request/status", json={"urls": urls, "refresh": True}
        )
...
def test_required_manual_job_is_pending_not_skipped() -> None:
    ...
    required = source._gitlab_check({"name": "deploy", "status": "manual", "allow_failure": False})
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"gitlab.acme.internal"},
            "https://evil-gitlab.acme.internal/a/b/-/merge_requests/1",
            False,
        ),
        (
            {"gitlab.acme.internal"},
            "https://gitlab.acme.internal.evil.test/a/b/-/merge_requests/1",
            False,
        ),
        ...
        (
            {"gitlab.acme.internal"},
            "https://www.gitlab.acme.internal/a/b/-/merge_requests/1",
            False,
        ),
    ],
)
...
async def test_fetch_issue_reserves_direct_fetch_capacity(monkeypatch) -> None:
    ...
    inflight = asyncio.ensure_future(source.fetch_issue("https://github.com/acme/repo/issues/12"))
    await started.wait()
    try:
...
async def test_issue_endpoint_audits_its_own_operation_name(monkeypatch, _mock_source_sel) -> None:
    ...
    operations = {
        call.kwargs.get("operation") for call in _mock_source_sel.log_api_access.call_args_list
    }
