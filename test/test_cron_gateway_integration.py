...
class TestCronUsageRow:
    """Issue #647: every model-spending cron turn builds one compatibility row
    tagged surface='cron'; zero-token script/command modes build none."""

    @pytest.mark.asyncio
    async def test_llm_cron_builds_usage_row_with_surface(self):
        gw = _make_gw_for_llm()
        job = _make_llm_job(model="claude-opus-4-8")
