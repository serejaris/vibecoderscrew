...
def _no_model_download(monkeypatch, _isolation_dirs):
    """Never let a test trigger an embedding-model download.

    A model download requires an explicit URL, but tests that boot the
    gateway/server startup path still use the opt-out as a safety net.
