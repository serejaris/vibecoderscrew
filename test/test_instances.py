...
class TestConfig:
    ...
    def test_recovery_knob_clamps(self):
        ...
        # explicit recovery compatibility seam. The boundary value itself is left untouched.
...
class TestSshTunnelManager:
    ...
    async def test_disconnect_and_shutdown(self, tmp_path):
        ...
        # shutdown preserves registry hints for the next explicit Connect/Retry
        await mgr.connect("cd-1")
        await mgr.shutdown()
...
class TestHandlers:
    ...
    def test_list_is_local_only_and_never_diagnoses(self, tmp_path, monkeypatch):
        from kiro_crew.dashboard import handlers_instances as handlers

        _enable(tmp_path, monkeypatch)
        reg = self._reg(tmp_path)
        reg.add(name="Persisted", ssh_host="host-a", instance_id="a")
        ...
        class LocalOnlyMgr:
            ...
            async def diagnose(self, iid):
                raise AssertionError("GET /api/instances must not diagnose or reach SSH")

        state = _State(reg, LocalOnlyMgr())
        body = _body(asyncio.run(handlers.api_instances_list(_FakeReq(state))))
        assert body["instances"][0]["status"]["state"] == "disconnected"
        assert body["instances"][0]["was_connected"] is True
    ...
    def test_connect_remints_when_stored_token_stale(self, tmp_path, monkeypatch):
        ...
        class FakeMgr:
            # An explicit Connect request can return a CONNECTED tunnel whose
            # stored token is stale (for example after a remote restart). The
            # gate probes, re-mints once, and serves the fresh token.
            def __init__(self):
                self.refreshed = []
...
class _ResilTunnel:
    """Controllable fake tunnel for explicit recovery tests."""
...
class TestExplicitRecoveryRefreshRestart:
    @pytest.fixture(autouse=True)
    def _free_ports(self, monkeypatch):
        import kiro_crew.instances.ssh_tunnel_manager as stm
    ...
    @pytest.mark.asyncio
    async def test_tunnel_drop_waits_for_explicit_reconnect(self, tmp_path, monkeypatch):
        reg, mgr = self._mgr(tmp_path)
        reg.add(name="CD", ssh_host="cd-1-alias", instance_id="cd-1")
        recovery_calls: list[str] = []
        monkeypatch.setattr(mgr, "_recover", lambda iid: recovery_calls.append(iid))
        task_calls: list[object] = []
        monkeypatch.setattr(asyncio, "create_task", lambda coro: task_calls.append(coro))

        mgr._on_tunnel_exit("cd-1")

        assert recovery_calls == []
        assert task_calls == []

    @pytest.mark.asyncio
    async def test_refresh_token_does_not_mint_after_drop(self, tmp_path):
        from kiro_crew.instances.ssh_tunnel_manager import TunnelState

        mint_calls: list[str] = []

        async def mint(host, *, remote_bin="", ttl="20h", remote_port=None, embed_parent_port=None):
            mint_calls.append(host)
            return "TOK"
        ...
        await mgr.connect("cd-1")
        mint_calls.clear()
        mgr._tunnels["cd-1"].status.state = TunnelState.DISCONNECTED
    ...
    async def test_refresh_token_once(self, tmp_path):
        ...
        await mgr.connect("cd-1")
        # Connect mints once and never schedules a background TTL task.
        assert not getattr(mgr, "_refresh_tasks", {})
        ...
        # Explicit refresh remains available; disconnect clears token metadata.
        await mgr.disconnect("cd-1")
        assert mgr.token_ttl_remaining("cd-1") is None

    @pytest.mark.asyncio
    async def test_refresh_passes_instance_remote_port(self, tmp_path):
        # F1 regression: connect AND explicit re-mint must target the instance's
        ...
        assert seen[-1] == 9001  # explicit refresh re-mints with the same port
...
class TestStartupNoRemoteEgress:
    """Startup creates local control-plane objects without remote I/O."""

    def test_enabled_startup_never_revives_persisted_hints(self, tmp_path, monkeypatch):
        import types
        ...
        class Manager:
            ...
            async def connect(self, instance_id):
                connects.append(instance_id)

            async def shutdown(self):
                return None

        monkeypatch.setattr(server, "KiroCrewConfig", types.SimpleNamespace(load=lambda: cfg))
        monkeypatch.setattr(server, "InstancesRegistry", lambda: reg)
        monkeypatch.setattr(server, "SshTunnelManager", Manager)
        app = web.Application()
        state = types.SimpleNamespace(instances_registry=None, instances_manager=None)
        server._register_instances_hooks(app, state, 5476)
        app.freeze()

        asyncio.run(app.on_startup.send(app))
        assert state.instances_registry is reg
        assert state.instances_manager.registry is reg
        assert connects == []

    def test_startup_does_not_schedule_tasks_or_mint_tokens(self, monkeypatch):
        import types
        ...
        monkeypatch.setattr(server, "KiroCrewConfig", types.SimpleNamespace(load=lambda: cfg))
        monkeypatch.setattr(server, "InstancesRegistry", Registry)
        monkeypatch.setattr(server, "SshTunnelManager", Manager)
        monkeypatch.setattr(server.asyncio, "create_task", lambda coro: task_calls.append(coro))
        app = web.Application()
        state = types.SimpleNamespace(instances_registry=None, instances_manager=None)
        server._register_instances_hooks(app, state, 5476)
        app.freeze()
