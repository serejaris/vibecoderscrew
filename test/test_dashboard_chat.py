...
class TestSlotLifecycle:
    ...
    @pytest.mark.asyncio
    async def test_list_slots_serves_cached_source_status_without_refresh(
...
class TestRunChatSegmentFlush:
    ...
    async def test_idle_turn_boundary_refreshes_source_status(self, tmp_path, monkeypatch):
        """Reaching idle at a turn boundary republishes cached PR/MR status.

        Regression for the production wiring: `_run_chat`'s idle branch still
        calls `state.refresh_slot_source_status(slot.key)`, but that method is
        cache-only. Explicit user refresh remains the provider read path.
        """
        from kiro_crew.providers.base import EVENT_COMPLETE, EVENT_TEXT_CHUNK, LLMEvent
...
class TestFolderCRUD:
    ...
    async def test_create_folder_rejects_multi_emoji_icon(self, tmp_path, monkeypatch):
        ...
        app = _make_folder_app(state)
        async with TestClient(TestServer(app)) as client:
            resp = await client.post("/api/chat/folders", json={"name": "Bad", "icon": "🚀🔥"})
            assert resp.status == 400
...
class TestSourceLinkUrlsExcludeIssues:
    ...
    async def test_slots_get_schedules_only_change_links(self, tmp_path, monkeypatch):
        ...
        assert len(links) == 2
        # ...and the slots endpoint remains cache-only.
        scheduler.assert_not_called()
