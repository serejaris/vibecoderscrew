...
def test_both_servers_defer_the_kiro_readiness_probe() -> None:
    """Boot entrypoints must not probe Kiro while mounting the server."""
    import inspect
    ...
    for func, name in (
        ...
        assert "warm_up()" not in inspect.getsource(
            func
        ), f"{name} reintroduced a provider/auth warm-up at startup"
