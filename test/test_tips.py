...
class TestSnoozedDocsSurvivesIdDrift:
    """Codex round-28: snooze survives legacy generated-id drift."""
...
class TestWeightedRandomSelection:
    ...
    def test_select_tip_legacy_ids_rank_by_doc_recency(self) -> None:
        """Codex round-14: legacy generated tips may carry custom ids and the
        catalog doc — recency must resolve via the doc field, preserving
        compatibility with older persisted state."""
        catalog = [
            CatalogEntry("Old", "S", "old.md", mtime=100.0),
            CatalogEntry("New", "S", "new.md", mtime=9999.0),
        ]
        # Legacy ids don't match catalog-derived ids; OLD doc listed FIRST so
        # the old insertion-order fallback would favor it.
        candidates = [
...
class TestStateFilePermissions:
    """Codex round-11 (HIGH): tips state must be owner-only."""
...
class TestTipFieldAllowlist:
    """Codex round-12 (HIGH): unknown/extra legacy fields never survive parse.
...
class TestStateStructuralValidation:
    ...
    def test_malformed_persisted_tip_fields_discarded(self, tmp_path: Path) -> None:
        ...
        with patch.dict(os.environ, {"KIROCREW_HOME": str(tmp_path)}):
            ...
            assert all(
                set(t.keys()) == set(("id", "feature", "title", "body", "why", "doc", "cta_prompt"))
                for t in st.tips
            )
...
class TestDocLevelDismissal:
    """Codex round-15: dismissal survives legacy id churn via the doc field."""
    ...
    def test_dismissed_doc_blocks_regenerated_tip_with_new_id(self) -> None:
        ...
        regenerated = {
            "id": "cron-scheduling-tip",  # fresh legacy-generated id
            "feature": "Cron",
            "title": "T",
            "body": "B",
            "why": "",
...
class TestConfigClamping:
    """Tests for local tips compatibility fields and ratio handling."""
    ...
    def test_tips_model_from_config_data(self) -> None:
        ...
        # The compatibility model slot is empty in the source-only release.
...
class TestCuratedTips:
    ...
    @pytest.mark.asyncio
    async def test_refresh_stays_local_and_does_not_open_background_session(self) -> None:
        import types
        ...
        class ForbiddenSessions:
            async def get_bg_session(self):
                raise AssertionError("tips must not open a background model session")

        cache = tips_mod.TipsCache(
            catalog=[CatalogEntry("Local Feature", "Use it locally.", "local.md")],
            state=TipsState(),
        )
        state = types.SimpleNamespace(_tips_cache=cache, sessions=ForbiddenSessions())
        tips = await tips_mod.generate_tips(state)
        assert [tip["id"] for tip in tips] == ["local-tip"]
