...
def test_provider_defaults_to_codex_and_preserves_explicit_acp() -> None:
    """Fresh profiles use Codex while existing ACP choices remain stable."""
    assert AgentConfig().provider == "codex"
    assert KiroCrewConfig().agent.provider == "codex"
    assert _load_from_dict({}).agent.provider == "codex"
    assert _load_from_dict({"agent": {}}).agent.provider == "codex"
    assert _load_from_dict({"agent": {"provider": "acp"}}).agent.provider == "acp"
...
class TestEdgeCases:
    ...
    **Validates: Requirements 2.4, 4.3, 4.4, 5.4**
    ...
    def test_embedding_model_help_requires_explicit_source(self) -> None:
        """Config help documents URL/path opt-in and keyword fallback."""
        url_help = MemoryConfig.__dataclass_fields__["embed_model_url"].metadata["help"]
        path_help = MemoryConfig.__dataclass_fields__["embed_model_path"].metadata["help"]
        assert MemoryConfig().embed_model_url == ""
        assert "no network fetch" in url_help
        assert "local GGUF" in path_help
        assert (
            "keyword/FTS"
            in MemoryConfig.__dataclass_fields__["embedding_provider"].metadata["help"]
        )
