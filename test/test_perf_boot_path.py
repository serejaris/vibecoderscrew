...
# ── Gateway boot: update check is explicit-action only ────────────────────


class TestGatewayUpdateCheckIsExplicit:
    """Gateway boot never runs the update check.

    Git fetches remain available through the explicit dashboard update action;
    boot only installs signal handlers and mounts local services.
    """
    ...
    def test_update_check_is_not_awaited_inline(self) -> None:
        ...
        assert "asyncio.create_task(self._check_for_updates())" not in src

    def test_signal_handlers_are_installed_without_update_work(self) -> None:
        src = inspect.getsource(GatewayOrchestrator.run)
        assert "loop.add_signal_handler(sig, _on_signal)" in src
        assert "asyncio.create_task(self._check_for_updates())" not in src

    def test_legacy_update_task_is_not_tracked_on_boot(self) -> None:
        run_src = inspect.getsource(GatewayOrchestrator.run)
        assert "self._background_tasks.add(self._update_check_task)" not in run_src
