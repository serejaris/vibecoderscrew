...
class TestOwnerSourceStatusTransport:
    ...
    async def test_websocket_initial_status_and_refresh_are_owner_only(
        ...
        if owner_request:
            ...
            # Connecting an owner window may expose a cached chip, but it never
            # starts a provider refresh. Refresh is an explicit user action.
...
class TestCachedCheckStatusTransport:
    """Owner transports expose cached PR status without automatic egress."""
    ...
    @pytest.mark.asyncio
    async def test_owner_ws_does_not_schedule_ttl_refreshes(self, monkeypatch) -> None:
        from kiro_crew.dashboard import ws as dashboard_ws
        from kiro_crew.dashboard.handlers import source_providers
        ...
        class FakeWebSocket:
            ...
            async def __anext__(self):
                # Keep the owner connection open across several former TTL
                # ticks. No provider scheduler should wake up.
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="allowlist changes are now pushed by explicit local events, never a timer"
    )
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(reason="periodic source refresh was intentionally removed")
...
class TestTurnBoundarySourceStatus:
    """Turn completion republishes cached PR state without provider egress."""
    ...
    def test_turn_boundary_republishes_cache_without_provider_refresh(
        self, state: DashboardState, monkeypatch
    ) -> None:
        from kiro_crew.dashboard.handlers import source_providers
        ...
        monkeypatch.setattr(state, "push_slots_update", push)
    ...
    def test_turn_boundary_swallows_refresh_failures(
        ...
        monkeypatch.setattr(
            state,
            "push_slots_update",
            MagicMock(side_effect=RuntimeError("socket closed")),
        )
