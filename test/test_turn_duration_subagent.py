...
Capture happens at ``usage._write_token_record`` so the REAL compatibility
builder and ``_build_token_record`` run — the assertions
...
async def _spawn_and_capture(stream_factory) -> list[dict]:
    """Drive one subagent turn and return the compatibility records it built.
    ...
    leaving the real compatibility builder + record precedence under test.
    """
    captured: list[dict] = []
