...
from kiro_crew.cli import _ensure_node
...
class TestNodePrerequisite:
    def test_default_check_does_not_execute_ensure_script(self, tmp_path, monkeypatch):
        """Gateway-style checks never run a script that could curl Node/mise."""
        script = tmp_path / "ensure-node.sh"
        script.write_text("curl https://example.invalid/node | bash\n", encoding="utf-8")
        script.chmod(0o755)
        ...
        def _run(*args, **kwargs):  # noqa: ANN002, ANN003
            ...
            raise AssertionError("check-only Node path must not spawn ensure-node.sh")

        monkeypatch.setattr("kiro_crew.cli.subprocess.run", _run)
        assert _ensure_node(str(tmp_path)) is False
        assert calls == []

    def test_explicit_install_path_runs_script(self, tmp_path, monkeypatch):
        script = tmp_path / "ensure-node.sh"
        ...
        monkeypatch.setattr("kiro_crew.cli.subprocess.run", run)
        assert _ensure_node(str(tmp_path), allow_install=True) is True
        assert run.call_args.args[0][:2] == ["bash", str(script)]
