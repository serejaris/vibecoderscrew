...
def test_per_turn_usage_persistence_is_removed_even_with_legacy_env(tmp_path, monkeypatch):
    shard_dir = tmp_path / "tokens"
    monkeypatch.setattr(usage_mod, "_TOKEN_USAGE_DIR", shard_dir)
    monkeypatch.setenv("KIROCREW_TOKEN_USAGE_PERSISTENCE", "1")
    monkeypatch.setenv("KIROCREW_TELEMETRY_ENABLED", "1")

    event = MagicMock()
    event.input_tokens = 1
    event.output_tokens = 1
    persist_token_record("slot", "model", event)
...
class TestPersistTokenRecord:
    def test_does_not_create_legacy_shard(self, tmp_path, monkeypatch):
        shard_dir = _patch_shard_layout(monkeypatch, tmp_path)
    ...
    def test_repeated_calls_do_not_create_a_shard(self, tmp_path, monkeypatch):
        shard_dir = _patch_shard_layout(monkeypatch, tmp_path)
    ...
    def test_writes_provider_field(self, tmp_path, monkeypatch):
        ...
        record = usage_mod._build_token_record(
            "slot", "opus", event, "claude_code", datetime.now(timezone.utc)
        )
    ...
    def test_provider_defaults_to_empty(self, tmp_path, monkeypatch):
        ...
        record = usage_mod._build_token_record(
            "slot", "opus", event, "", datetime.now(timezone.utc)
        )
...
class TestPersistTokenRecordAsync:
    @pytest.mark.asyncio
    async def test_async_does_not_create_legacy_shard(self, tmp_path, monkeypatch):
        shard_dir = _patch_shard_layout(monkeypatch, tmp_path)
        monkeypatch.setenv("KIROCREW_TOKEN_USAGE_PERSISTENCE", "1")
        monkeypatch.setenv("KIROCREW_TELEMETRY_ENABLED", "1")
        event = MagicMock()
        ...
        today = datetime.now().astimezone().strftime("%Y-%m-%d")
        assert not (shard_dir / f"{today}.jsonl").exists()
...
class TestBuildTokenRecordContextFields:
    ...
    def test_build_record_keeps_new_fields_without_persistence(self):
        record = usage_mod._build_token_record(
            "slot",
            "opus",
            self._event(),
...
class TestModelSourceFallback:
    ...
    def test_fallback_fills_empty_model(self):
        assert usage_mod._resolve_model("", _Inner("claude-opus-4.8")) == "claude-opus-4.8"

    def test_explicit_model_wins(self):
        assert (
            usage_mod._resolve_model("claude-haiku-4.5", _Inner("claude-opus-4.8"))
            == "claude-haiku-4.5"
        )

    def test_no_source_leaves_model_empty(self):
        assert usage_mod._resolve_model("", None) == ""

    def test_unusable_source_leaves_model_empty(self):
        # A test double with no model attr must not break model resolution.
        assert usage_mod._resolve_model("", object()) == ""

    def test_auto_sentinel_is_treated_as_unresolved(self):
        ...
        assert usage_mod._resolve_model("auto", _Inner("claude-opus-4.8")) == "claude-opus-4.8"

    def test_auto_without_resolvable_source_records_blank(self):
        ...
        assert usage_mod._resolve_model("auto", None) == ""
