...
class TestProviderDetection:
    def test_default_is_codex(self, tmp_path):
        with patch("pathlib.Path.home", return_value=tmp_path):
            assert _get_provider_type() == "codex"
    ...
    def test_preserves_explicit_unsupported_provider(self, provider):
        assert _get_provider_type({"agent": {"provider": provider}}) == provider
...
class TestReadConfig:
    ...
    def test_parsers_survive_non_dict_sections(self, bad):
        ...
        assert _get_provider_type(bad) == "codex"
...
class TestLLMPoolStart:
    @pytest.mark.asyncio
    async def test_codex_config_fails_closed_without_acp_spawn(self, tmp_path):
        config = tmp_path / ".kirocrew" / "config.json"
        config.parent.mkdir(parents=True)
        config.write_text('{"agent": {"provider": "codex"}}')

        with (
            patch("pathlib.Path.home", return_value=tmp_path),
            patch(
                "kiro_crew.knowledge.llm_pool.AcpWorker",
                side_effect=AssertionError("Codex must not construct an ACP worker"),
            ) as acp_worker,
        ):
            pool = LLMPool(pool_size=1)
            with pytest.raises(KnowledgeProviderUnavailable) as exc_info:
                await pool.start()

        assert exc_info.value.code == "knowledge_provider_unavailable"
        assert exc_info.value.provider == "codex"
        assert pool._started is False
        assert pool._workers == []
        acp_worker.assert_not_called()

    @pytest.mark.asyncio
    @pytest.mark.parametrize("provider", ["", "bogus"])
    async def test_unsupported_provider_fails_closed_without_any_worker_spawn(
        self, provider, tmp_path
    ):
        config = tmp_path / ".kirocrew" / "config.json"
        config.parent.mkdir(parents=True)
        config.write_text(json.dumps({"agent": {"provider": provider}}))

        with (
            patch("pathlib.Path.home", return_value=tmp_path),
            patch(
                "kiro_crew.knowledge.llm_pool.AcpWorker",
                side_effect=AssertionError("unsupported provider must not construct ACP"),
            ) as acp_worker,
            patch(
                "kiro_crew.knowledge.llm_pool.CCWorker",
                side_effect=AssertionError("unsupported provider must not construct CC"),
            ) as cc_worker,
        ):
            pool = LLMPool(pool_size=1)
            with pytest.raises(KnowledgeProviderUnavailable) as exc_info:
