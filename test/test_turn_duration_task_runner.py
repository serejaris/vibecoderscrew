...
``task_executor`` builds a compatibility usage row at two sites — the main
...
``_build_token_record`` is exercised for real); the compatibility writer seam
...
def _intercept_usage(monkeypatch: pytest.MonkeyPatch) -> list[dict]:
    ...
    Patches the compatibility writer seam and the client-reading helpers (which would
