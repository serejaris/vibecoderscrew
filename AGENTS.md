...
- OSS-flipped defaults (keep): embeddings use an **in-process** runtime when a
  model is available (vendored llama-cpp-python under `_vendor/`; Qwen3 GGUF
  downloads require an explicit HTTPS URL via `KIROCREW_EMBED_MODEL_URL` /
  `memory.embed_model_url`, or an explicit local `memory.embed_model_path`; there
  is no default URL/download and absent models fall back to keyword/FTS search;
  the `EmbeddingBackend` seam keeps other runtimes possible); voice TTS defaults to
  **Piper** (local); Slack enterprise gate is
...
| `embeddings.py` | In-process embedding runtime (vendored llama-cpp-python) behind the `EmbeddingBackend` ABC (swap seam via `register_embedding_backend`); explicit model URL/path resolution, keyword/FTS fallback, and LRU embed cache (128 entries, keyed by text + model_id) |
