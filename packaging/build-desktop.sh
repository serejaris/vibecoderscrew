...
# Build the standalone VibecodersCrew desktop app end-to-end.
#
...
case "$KC_VERSION" in
  *-nightly.*)
    PRODUCT_NAME="VibecodersCrew Nightly"
    APP_ID="dev.serejaris.vibecoderscrew.nightly"
    ;;
  *)
...
( cd "$ELECTRON_DIR"
  ...
  if [ "$PRODUCT_NAME" = "VibecodersCrew Nightly" ]; then
    # Nightly keeps a distinct bundle id so macOS TCC, notifications, URL
    # handlers, and update metadata cannot collide with the stable app.
    EB_ARGS+=(
...
echo ""
echo "    The .app embeds the backend, so it runs with no PATH vibecoderscrew needed."
