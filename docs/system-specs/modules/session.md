...
breaker, per-session semaphore, and a lazily-created background session.
...
- **Created on the first explicit background action** (or an explicitly
  requested `start_pool()`), never as a gateway/UI startup side effect
...
- **kiro (`acp`)** — the only backend the multiplexed `AcpRuntime` supports;
  selected when `agent.provider=acp` is explicit.
...
- **non-kiro** — falls back to a `_ProviderBgSession` over the shared
  ...
  kiro-only, so any non-kiro backend must use the provider path. The canonical
  `codex` provider uses this fallback; the ACP path remains available through
  the explicit `agent.provider=acp` setting.
...
Every persisted mapping must carry an explicit provider label (`acp`, `codex`,
or `claude_code`). Legacy entries that predate this field fail closed: they
are not resumed and their native files are not treated as ACP files. The next
fresh turn writes the current provider label. This prevents a Codex default
from accidentally loading or deleting an incompatible ACP transcript.
...
```
explicit background action / opt-in start_pool()
...
- **Before provider startup**: `start_pool()` calls it before spawning any sessions
