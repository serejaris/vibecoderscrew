...
`src/kiro_crew/cloud/` optionally runs VibecodersCrew on the user's **own** AWS EC2 instance with a
...
- (2) the shell `deniedCommands` in `config/defaults.json` (kiro-cli
  ...
  the `vibecoderscrew cloud destroy|stop|start|launch|connect|tunnel|login` wrappers
...
| `iam.py` | Least-privilege launcher policy generator (applied by the user, never by VibecodersCrew) + read-only reachability check + the **content-fixed instance permissions-boundary document** (`boundary_policy_document`/`boundary_arn`) and its constants (`BOUNDARY_NAME`). |
...
For deterministic deployments, the launcher packages the local checkout and
uploads it to a launcher-owned bucket
...
- **IMDSv2 enforced** on the instance (`HttpTokens: required`, hop limit 1):
  VibecodersCrew runs a prompt-injectable agent, so an SSRF/injection that reaches the
...
- **Escalation primitives constrained + immutable, pre-created permissions
  ...
  - **Residual (first-write race), tracked in as-built:** the very first
    ...
    hole. Operators who want it gone entirely run `vibecoderscrew cloud iam-boundary`
...
The public source install is `install.sh`; it does not provision cloud
infrastructure. The optional cloud launcher is an operator action: install the
AWS CLI and Session Manager plugin using their official guides, then run
`vibecoderscrew cloud launch`. The gateway always runs on the Linux EC2 box,
never on the Windows client.

`vibecoderscrew cloud launch` runs `python -m kiro_crew`, which imports the whole CLI —
