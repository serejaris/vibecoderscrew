...
Last Updated: 2026-08-07 (startup browser launch is explicit opt-in; no provider warm-up or telemetry beacon worker; prior: slack-link fresh-anchor title fallback chain)
...
### `run_gateway(cfg: KiroCrewConfig, *, no_dashboard=False, no_crons=False, no_open=False, open_browser=False) -> None`
Starts the Socket Mode listener. Blocks until SIGINT/SIGTERM. Browser launch is
opt-in: `--open` passes `open_browser=True`; `dashboard.auto_open_browser` is
false by default and is honored only when explicitly configured. The legacy
`--no-open` flag remains a compatibility suppressor. When `no_crons=True`, the
`CronService` is instantiated but not started — cron jobs are visible in the
dashboard but not executed. Use for multi-instance setups where a single
primary instance handles cron execution. On shutdown, calls
`dashboard_state.close_all_ws()` before `AppRunner.cleanup()` to prevent 30s
hang from blocked WebSocket `async for msg` loops.
...
The gateway does not call `start_pool()`, run the update checker, or probe MCP
servers during startup. Background sessions and warm-pool providers are
created lazily when an explicit turn/action first needs one. Chat sessions
cold-start on first message — no startup provider spawn, no warm pool, no MCP
reset hack. `/api/status` and the default MCP GET endpoints return cached or
configuration data only; `/api/update/check` and the explicit MCP Probe action
are the user-authorized live operations. Startup never schedules the legacy
telemetry beacon or opens a browser unless `--open` or an explicit
`dashboard.auto_open_browser=true` setting authorizes that action.
