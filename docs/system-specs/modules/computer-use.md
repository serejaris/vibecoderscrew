...
> the compression pipeline and the security floors are all VibecodersCrew's own.
...
```
kiro-cli
  └─ spawns  vibecoderscrew mcp-computer      (stdio MCP server: kirocrew-computer)
...
`computer_end_turn` is control-plane (it drops VibecodersCrew's *own* cached snapshots
...
- `permissions` comes from shelling `vibecoderscrew computer doctor --json`
...
| VibecodersCrew's own window | `policy.check_app` | Driving our own Settings UI would let the agent click the enable above — a self-escalation loop that would make the keystone pointless. Matched on bundle id, process name **and window title** — see below. |
...
VibecodersCrew's own desktop app appear permanently empty. Order of operations:
...
clipboard is out of band: VibecodersCrew never reads it, so nothing can classify what it
...
- **Refusals go through the SAME exit as results** (`tools._refusal`). A refusal is
  ...
  Refusals that are 100% VibecodersCrew's own static prose (the primary-enable refusal, the
...
`vibecoderscrew computer doctor --json` is what the gateway shells for the Settings
...
## The CLI (`vibecoderscrew computer`) — and what is deliberately not ported
...
a per-process `SnapshotIndex` with a 90s TTL. Two separate `vibecoderscrew computer call`
...
| `turn-ended [--previous-notify]` (a host notify hook) | same intent, different shape: `computer_end_turn` is an MCP tool, so the model drops its own snapshot cache rather than relying on a host lifecycle hook VibecodersCrew does not have. |
...
* once the operator enables the feature, prompt injection that reaches the agent
  ...
  to contain it;
* the one structural defence is that the agent cannot drive VibecodersCrew's own window
  ...
  merely contains "vibecoders crew" is refused — an editor with this repo open, a browser
...
| `computer_use/policy.py` | The one retained app refusal (VibecodersCrew's own window) + the operator's allow/deny lists, secure-target + text refusals, the click-target/method/button refusals + `resolve_click_method`, `redact_result` |
...
| `computer_use/cli.py` | `vibecoderscrew computer doctor [--json] \| apps \| call` |
| `mcp_computer.py` | The thin stdio shim (`vibecoderscrew mcp-computer`) |
