...
- The dashboard `/api/models` default response is a side-effect-free static/cache
  catalog. A live one-shot subprocess refresh is available only with the
  explicit `?refresh=1` marker and validates completion before parsing stdout:
  nonzero exit (with a bounded, redacted stderr tail), empty stdout, malformed
  JSON, or a payload without a model list each returns HTTP 503. A subprocess
  failure is never misreported as `JSONDecodeError` or cached as a successful
  empty model list.
- **Provider refreshes are explicit-action only.** `kiro-cli` auto-launches an
  interactive browser login for any subcommand run unauthenticated
  (`--no-interactive` does not suppress it; there is no opt-out env var). The
  model and billing routes therefore return static/cache data on ordinary UI
  mounts and invoke `reject_if_kiro_unverified()` only for an explicit
  `?refresh=1` request. The billing route awaits one bounded explicit refresh
  and returns its result/error in that request; it never schedules a TTL/timer
  refresh.
  The model list has no degraded 8s polling loop. Ordinary sends are ungated,
  because a failing ACP attempt reports its own `AcpAuthRequired`, while an
  explicit refresh has no turn to carry that error and must fail closed.
  Explicit refreshes authorize on a **freshly verified** probe
  (`verified_ready`, 30s ceiling), never the bare latch.
- **`AcpAuthRequired` is the authoritative logout signal.** Readiness is probed
  only on an explicit user action, so a mid-session sign-out is
