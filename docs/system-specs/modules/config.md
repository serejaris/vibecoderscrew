...
class AgentConfig:
    ...
    provider: str = "codex"        # "codex" (Codex App Server) or "acp" (kiro-cli)
...
``agent.provider`` defaults to ``"codex"`` for fresh profiles and when the
key is absent from an existing config. An explicit ``"acp"`` value is
preserved during load, so KiroACP remains an opt-in compatibility path.
