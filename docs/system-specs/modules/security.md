...
> to the `claude-agent-acp` backend and was **deleted** when that standalone
> backend was removed. On the explicit KiroACP/`kiro-cli` path
> (`agent.provider=acp`), kiro-cli's
