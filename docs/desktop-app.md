...
# VibecodersCrew Desktop App

> VibecodersCrew uses the stable application identity
> `dev.serejaris.vibecoderscrew` (`.nightly` for nightly builds) / `VibecodersCrew.app`, ships no signed binaries,
...
The desktop app is an optional local-development [Electron](https://www.electronjs.org/) shell that wraps
the Vibecoders Crew web dashboard and embeds a **self-contained Python backend**. The
...
double-click the locally built app and the dashboard opens. Local desktop output
is not attached to or advertised by source-only releases.
...
## What `make desktop` produces locally

```bash
make desktop               # local macOS/Linux desktop build
UNIVERSAL=0 make desktop   # local macOS host-arch build
```
...
|---------|----------|----------|
| `make desktop` | macOS | `VibecodersCrew-<version>-universal.dmg` |
| `UNIVERSAL=0 make desktop` | macOS | `VibecodersCrew-<version>-arm64.dmg` (Apple Silicon host) or `VibecodersCrew-<version>.dmg` (Intel host) |
| `make desktop` | Linux | `VibecodersCrew-*.AppImage` (host arch) |

The electron-builder configuration lives in
[`website/electron/package.json`](../website/electron/package.json):

- **appId:** `dev.serejaris.vibecoderscrew` (nightly: `dev.serejaris.vibecoderscrew.nightly`)
- **productName:** `VibecodersCrew`
- macOS display name: `Vibecoders Crew` via `CFBundleDisplayName`; `CFBundleName`
...
On macOS, `make desktop` produces a single `VibecodersCrew-<version>-universal.dmg`
...
Keep these outputs local. The source-only release has no desktop artifact
...
```bash
V=<version>
hdiutil attach -nobrowse -readonly "website/electron/dist/VibecodersCrew-$V-universal.dmg"
APP="/Volumes/VibecodersCrew $V-universal/VibecodersCrew.app"

# 1. The shell binary is fat:
lipo -archs "$APP/Contents/MacOS/VibecodersCrew"
#   → x86_64 arm64
...
hdiutil detach "/Volumes/VibecodersCrew $V-universal"
```
...
**CI note:** this public fork has no desktop build or publish workflow. Any
`make desktop` run is an operator-local development check; no artifact is
uploaded, signed, notarized, or offered through an update feed.
...
(e.g. a `VibecodersCrew-<old-version>.dmg` from before a version bump, or a stale `mac/`
...
rm -f VibecodersCrew-<old-version>*.dmg      # stale DMGs from a prior version
...
## Historical code-signing reference (macOS)

The public v1.0.1 release is source-only and makes no signing or notarization
claim. The details below describe optional local/upstream packaging provenance;
they are not release steps for this fork.

An unsigned `.app`/DMG is quarantined by Gatekeeper and shows **"VibecodersCrew is
...
after right-click → Open or `xattr -dr com.apple.quarantine VibecodersCrew.app`.)
...
Verify the result: `spctl -a -vv "VibecodersCrew.app"` should report
...
an app reads one of them, macOS shows a modal *"VibecodersCrew would like to access
...
Nothing VibecodersCrew does at startup needs those folders. They were only ever
...
- **Explicit access is unaffected.** If you point VibecodersCrew at a project inside
...
An upstream signed deployment can keep grants sticky across updates; the
VibecodersCrew source release makes no such distribution claim.
...
> **no** system prompt ever appears, and there is no VibecodersCrew row under System
...
`dev.serejaris.vibecoderscrew` whose `csreq` matches a cdhash the Developer-ID release
...
```bash
tccutil reset Microphone dev.serejaris.vibecoderscrew
```

This is also why a stable identity matters for any upstream signed deployment;
the source-only release does not distribute that deployment.
...
`ssh <host> vibecoderscrew token` on each launch instead of starting a local backend.
