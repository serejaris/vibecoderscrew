...
async def _synthesize_polly(
    ...
    # Do not probe PATH before spawning: callers and tests may provide a
    # hermetic executable resolver, while an absent CLI still degrades through
    # the ``FileNotFoundError``/``OSError`` handler below.  ``is_available`` is
    # the side-effect-free probe for UI readiness checks.
