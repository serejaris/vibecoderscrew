...
def safe_write_file_nolink(
    ...
    # The xattr functions are POSIX-only in typeshed even though CPython
    # exposes them on supported filesystems. Resolve them dynamically so this
    # module remains importable on Windows and mypy accepts the guarded calls.
    listxattr = getattr(os, "listxattr", None)
    getxattr = getattr(os, "getxattr", None)
    setxattr = getattr(os, "setxattr", None)
    if callable(listxattr) and callable(getxattr) and callable(setxattr):
        listxattr_fd = cast("Callable[[int], list[str]]", listxattr)
        getxattr_fd = cast("Callable[[int, str], bytes]", getxattr)
        try:
            for _attr in listxattr_fd(fd):
                src_xattrs.append((_attr, getxattr_fd(fd, _attr)))
        except OSError as exc:
    ...
    try:
        ...
        try:
            ...
            setxattr_fd = cast("Callable[[int, str, bytes], None]", setxattr)
            for attr, value in src_xattrs:
                try:
                    setxattr_fd(tfd, attr, value)
                except OSError:
