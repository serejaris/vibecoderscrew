...
The topbar shows a billing readout only when the active provider advertises the
`usageBilling` capability. Providers such as Codex do not expose the Kiro
credit readout and the dashboard never calls their missing usage endpoint.

The Developer → Telemetry route and Settings → Privacy surface are static
notices: telemetry, install receipts, polling, and legacy token analytics are
disabled. Prompts, files, and credentials stay local; no legacy telemetry rows
are collected.
