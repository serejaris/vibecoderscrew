...
runtime pip install. When an operator explicitly configures
``KIROCREW_EMBED_MODEL_URL`` or ``memory.embed_model_url`` (HTTPS only), the
Qwen3-Embedding-0.6B GGUF can be downloaded in the background, sha256-verified,
and installed persistently to ``~/.kiro/crew/models/``. A byte-identical model
already present in a legacy Ollama store may be salvaged locally. There is no
default URL or first-install network fetch; without a configured source,
memory stays on keyword/FTS search.
...
While the model file is absent (first boot, an explicitly configured download
in flight, or a failed download) every embed call returns ``None`` and memory
degrades gracefully to keyword/FTS search — the existing lazy-rebind machinery
in ``vector_memory._try_embed`` picks embeddings up automatically once the
...
_DOWNLOAD_MAX_ATTEMPTS = 6  # explicit-URL startup task (long backoff, may span hours)
...
# Safety switch for tests/CI: never kick a large model download from a test run.
_SKIP_DOWNLOAD_ENV = "KIROCREW_SKIP_MODEL_DOWNLOAD"
# Distribution: a downstream operator supplies an HTTPS URL (for example, an
# internal mirror). There is no hard-coded CDN or first-install source. The
# sha256 pin above is the trust anchor for every network payload, so a tampered
# object fails verification. Resolution order is the explicit
# KIROCREW_EMBED_MODEL_URL env var, then memory.embed_model_url, then empty.
...
# Operators must set KIROCREW_EMBED_MODEL_URL or memory.embed_model_url for a
# network download, or memory.embed_model_path for a local GGUF.
_DEFAULT_MODEL_URL = ""
_HTTP_TIMEOUT_SECS = 1800  # large GGUF transfer; slower links retry with backoff
...
def models_dir() -> Path:
    """Directory for managed embedding models (respects KIROCREW_HOME)."""
    return config_dir() / _MODELS_DIR_NAME


def default_model_path() -> Path:
    """Path of the managed model download target when a URL is configured."""
...
def build_gated_bundled() -> LlamaCppEmbedder:
    ...
    there is nothing to adopt — but the file can still be ABSENT until an
    explicit model URL download, which is why the revert must also prove the
    model loads before its config is persisted.
    """
...
def embedding_model_is_custom() -> bool:
    ...
    Gates the managed default-model download: a configured custom model must
    never be replaced by the bundled one, and a BROKEN custom path must not
    silently fall back to it either.
    """
...
def model_file_present(path: Path | None = None) -> bool:
    ...
    specifically mean the managed download target (the download manager) pass
    ...
    def _kick_background_load(self) -> None:
        ...
        with self._load_lock:
            ...
            if not model_file_present(self._model_path):
                # Not a failure — an explicitly configured download may still be in flight.
                return
...
async def _run_download_on_daemon_thread(fn: "Callable[[], tuple[bool, str]]") -> tuple[bool, str]:
    ...
    exit — an in-flight large model download would hang a Ctrl-C or a finished
...
class ModelDownloadManager:
    """Optional background download of the embedding GGUF from an explicit URL.

    When a URL is configured, the gateway may kick ``ensure_model()`` as a
    background task at startup, so boot is never blocked by the large transfer.
    ``status`` is a plain dict readable at any time by the dashboard status
    endpoint. Concurrent ``ensure_model()`` calls (startup task + a dashboard
    Enable click) share one in-flight download via ``_lock``.
    """
    ...
    async def ensure_model(self, attempts: int = 1) -> bool:
        """Download the model if absent and explicitly configured.

        Returns True when the file is in place.
        ...
        retry also happens on an explicitly configured gateway startup and on
        each dashboard Retry click). Skipped entirely (returns False) when the
        ``KIROCREW_SKIP_MODEL_DOWNLOAD=1`` escape hatch is set — test runs
        must never trigger a large model download.
        """
        if self.model_ready():
    ...
    def _salvage_legacy_ollama_blob(self) -> bool:
        ...
        the large transfer. Best-effort: any failure falls through to the
        explicitly configured HTTPS download; the copy is sha256-verified like
        a network download.
        """
    ...
    def _download_once(self) -> tuple[bool, str]:
        """Try sources in order: local Ollama salvage → explicit HTTPS URL."""
        ...
        if self._salvage_legacy_ollama_blob():
            return True, ""
        # HTTPS download from the operator-configured URL. Reports byte-level
        # progress and never runs when the URL resolver returns an empty value.
        return self._download_via_https()

    def _download_via_https(self) -> tuple[bool, str]:
        """Download the GGUF from the configured HTTPS URL with progress reporting."""
...
def start_background_model_download() -> "asyncio.Task[bool] | None":
    """Start an optional explicit-URL background model download.

    Returns the task or ``None`` when no download is needed or no source is
    configured.

    Called from gateway/server startup. A custom model never triggers the
    managed download, and an empty URL leaves embeddings on keyword/FTS search.
    Idempotent: a second call while an explicit download is in flight returns
    the existing task instead of spawning another.
    """
    ...
    if not _resolve_model_url():
        logger.info("No embedding model URL configured — background download not started")
        return None
    if _download_task is not None and not _download_task.done():
...
def make_sync_embed_fn() -> Callable[[str], "list[float] | None"]:
    ...
    (None) are not cached so an explicitly configured model still loading can
    be retried. Embedding never blocks on the model load (kicked in the
    background); callers get
