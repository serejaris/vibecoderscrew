...
def ensure_cli_shims_on_path(bin_dir: Path | None = None) -> tuple[str, ...]:
    """Ensure the canonical CLI shim and its optional compatibility alias exist.

    ``vibecoderscrew`` is the public command. ``kirocrew`` remains a technical
    compatibility alias for existing agent specs and scripts. Both shims point
    at the same validated executable when the install exposes a concrete target;
    an unrelated executable already occupying the alias is left untouched.

    Returns the paths created or updated during this call. An empty tuple means
    that no usable target was available or both names were already valid.
    """
    target = _resolve_kirocrew_bin()
    if not (os.path.isabs(target) and os.path.isfile(target) and os.access(target, os.X_OK)):
        return ()
    if _in_linked_git_worktree(Path(target).resolve()):
        logger.info(
            "Not installing CLI shims: %s is inside a linked git worktree; "
            "install from the primary clone instead",
            target,
        )
        return ()

    destination = bin_dir or (Path.home() / ".local" / "bin")
    changed: list[str] = []
    for command in ("vibecoderscrew", "kirocrew"):
        link = destination / command
        ...
        try:
            if link.is_symlink():
                ...
                # Never replace an operator-owned executable with a compatibility
                # alias. The canonical name can still be validated through PATH.
                if command == "kirocrew":
                    continue
            ...
            logger.warning("Could not create %s shim at %s", command, link, exc_info=True)
...
def run_first_run_setup() -> None:
    ...
    * **PATH shims** — ``ensure_cli_shims_on_path()`` is idempotent and writes
      the public ``vibecoderscrew`` launcher plus the optional ``kirocrew``
      compatibility alias, so the packaged app has the same command surface.
    * **Stale predecessor MCP purge** — ``clean_stale_managed_mcp()`` mutates
    ...
    try:
        ...
        if shims:
            logger.info("First-run: linked CLI shims at %s", ", ".join(shims))
    except Exception:
