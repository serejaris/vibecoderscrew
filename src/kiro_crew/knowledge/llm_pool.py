...
class KnowledgeProviderUnavailable(RuntimeError):
    """The selected provider has no Knowledge Library worker implementation."""
    ...
    def __init__(self, provider: str, *, code: str | None = None) -> None:
        ...
        if code is not None:
            self.code = code
        if self.code == "unsupported_provider":
            message = (
                f"Unsupported Knowledge LLM provider={provider!r}; "
                "select agent.provider=acp explicitly for the ACP worker."
            )
        else:
            message = (
                f"Knowledge LLM worker is unavailable for provider={provider!r}; "
                "select agent.provider=acp explicitly for the ACP worker."
            )
...
def _get_provider_type(config: Optional[dict] = None) -> str:
    """Configured knowledge provider; defaults to ``"codex"``."""
    data = _read_config() if config is None else config
    agent = _section(data, "agent")
    if "provider" not in agent:
        return "codex"
    provider = agent.get("provider")
    # Preserve an explicit empty/malformed value so LLMPool can fail closed
    # with ``unsupported_provider`` instead of silently selecting ACP/Codex.
...
class LLMPool:
    ...
    async def _create_worker(self) -> Worker:
        ...
        if self._provider_type == "codex":
            # Knowledge workers have no Codex App Server adapter yet. Fail
            # closed here: silently constructing AcpWorker would launch
            # kiro-cli for a Codex profile, violating the explicit-provider
            # boundary. EntityExtractor turns this into its local empty
            # extraction fallback; URL fetchers surface this capability error.
            raise KnowledgeProviderUnavailable(self._provider_type)
        if self._provider_type == "claude_code":
            worker: Worker = CCWorker()
        elif self._provider_type == "acp":
            worker = AcpWorker(sandbox_mode=self._sandbox_mode)
        else:
            # Never treat a typo, empty value, or future provider id as ACP.
            # Worker construction is deliberately below this guard so an
            # unsupported profile cannot spawn any provider process.
