...
# Compatibility-only endpoint slot for the legacy beacon (see
# kiro_crew/beacon.py). It is intentionally empty in the source-only release;
# the outbound telemetry gate remains hard-disabled even if old config data
# supplies a value. Keeping the slot here avoids a config import edge.
...
# Public provider boundary. Every runtime dispatch site must select one of
# these ids explicitly; an unknown value must surface as a configuration error
# instead of silently taking the ACP branch.
SUPPORTED_PROVIDER_IDS = frozenset({"acp", "codex"})
...
class MemoryConfig:
    embedding_provider: str = field(
        ...
        metadata=_meta(
            "Embedding Provider",
            "In-process embedding backend via vendored llama-cpp-python. The runtime stays "
            "available with no model configured and falls back to keyword/FTS search. "
    ...
    embed_model_url: str = field(
        ...
        metadata=_meta(
            "Embedding Model URL",
            "Explicit HTTPS URL for the embedding model GGUF (mirrored/airgapped "
            "deployments). Empty means no network fetch; set this value or "
            "KIROCREW_EMBED_MODEL_URL to opt into a sha256-verified download.",
        ),
    )
    embed_model_path: str = field(
        ...
        metadata=_meta(
            ...
            "Absolute path to a local GGUF embedding model. When set, no network model is "
            "downloaded or installed. Set embedding_dim to the model's output width. "
            "Changing the model changes the vector space, so stored embeddings are "
            "regenerated automatically. The KIROCREW_EMBED_MODEL_PATH env var wins over "
            "this.",
        ),
    )
...
class DashboardConfig:
    ...
    auto_open_browser: bool = field(
        ...
        metadata=_meta(
            "Auto Open Browser",
            "Compatibility setting. Browser launch is opt-in via the explicit gateway CLI action.",
        ),
    )
    ...
    tips_model: str = field(
        ...
        metadata=_meta(
            "Tips Model",
            "Compatibility field retained for config round-trips; local catalog tips do not invoke a model.",
        ),
    )
    tips_explore_ratio: float = field(
        ...
        metadata=_meta(
            "Tips Explore Ratio",
            "Probability of picking a random catalog tip (0-1). Higher values broaden local discovery.",
        ),
    )
...
class TelemetryConfig:
    ...
    local_dir: str = field(
        ...
        metadata=_meta(
            "Local Metrics Dir",
            "Compatibility field. Per-process metric JSONL persistence is disabled in "
            "the source-only release and this path is ignored.",
        ),
    )
    export_interval_seconds: int = field(
        ...
        metadata=_meta(
            "Export Interval (s)",
            "Compatibility field retained for config round-trips; no local exporter "
            "runs in the source-only release.",
        ),
    )
    retention_days: int = field(
        ...
        metadata=_meta(
            "Retention (days)",
            "Compatibility field retained for config round-trips; no metric shards "
            "are written in the source-only release.",
        ),
    )
...
class InstancesConfig:
    ...
    warm_set_cap: int = field(
        ...
        metadata=_meta(
            ...
            "at once. Least-recently-used iframes beyond this are evicted from the "
            "viewport; tunnels remain under explicit owner control. Bounds "
            "memory/socket use (each warm instance is a full dashboard SPA).",
        ),
    )
    ...
    max_recovery_attempts: int = field(
        ...
        metadata=_meta(
            ...
            "Compatibility limit for the private explicit recovery seam. Normal "
            "tunnel drops remain disconnected until the owner clicks Connect/Retry.",
        ),
    )
    recover_backoff_max_secs: float = field(
        ...
        metadata=_meta(
            ...
            "Compatibility cap for the private explicit recovery seam. No background "
            "task consumes this value.",
        ),
    )
    probe_failure_threshold: int = field(
        ...
        metadata=_meta(
            ...
            "(zombie) tunnel is torn down and left disconnected for explicit Retry.",
        ),
    )
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_recovery_attempts < 1:
            ...
            logger.warning(
                ...
                "(guards the explicit recovery compatibility seam)",
                self.max_recovery_attempts,
...
class KiroCrewConfig:
    ...
    auto_update: bool = field(
        ...
        metadata=_meta(
            "Auto Update",
            "Compatibility field. Automatic update checks are disabled in this "
            "source-only release; updates require an explicit user action.",
        ),
    )
    timezone: str = field(
        default="",
    ...
    def load(cls) -> KiroCrewConfig:
        ...
        cfg = cls(
            agent=AgentConfig(
                ...
                # Missing provider is a fresh-profile default. Preserve an
                # explicit legacy ``acp`` value verbatim for existing installs.
            ...
            dashboard=DashboardConfig(
                ...
                auto_open_browser=dashboard_data.get("auto_open_browser", False),
                ...
                tips_model=str(dashboard_data.get("tips_model", "")),
    ...
    def create_provider_factory(self) -> Callable:
        ...
        """
        if self.agent.provider not in SUPPORTED_PROVIDER_IDS:
            raise ValueError(
                f"Unsupported agent.provider {self.agent.provider!r}; " "choose 'codex' or 'acp'."
            )
