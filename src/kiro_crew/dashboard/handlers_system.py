...
async def api_status(request: web.Request) -> web.Response:
    ...
    # Status is a cache/config-only read.  Remote update checks are explicitly
    # owned by ``GET /api/update/check`` and must never be triggered by the
    # dashboard's frequent status poll.
...
def _get_static_system_info() -> dict[str, object]:
    ...
    # ``/api/status`` is polled during dashboard boot and must not spawn a
    # platform command.  ``platform.machine()`` is sufficient for the public
    # architecture label; the detailed live metrics endpoint may still use
    # platform-specific samplers when the user opens System.
    arch = platform.machine()
    if sys.platform == "darwin" and arch.lower() in {"arm64", "aarch64"}:
    ...
    # Total memory (static) — cross-platform.  Keep this path free of command
    # execution so status remains side-effect free on every platform.
    if sys.platform == "darwin":
        try:
            page_size = int(os.sysconf("SC_PAGE_SIZE"))
            page_count = int(os.sysconf("SC_PHYS_PAGES"))
            if page_size > 0 and page_count > 0:
                info["mem_total_gb"] = round(page_size * page_count / (1024**3), 1)
