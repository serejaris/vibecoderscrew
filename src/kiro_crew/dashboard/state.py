...
class DashboardState:
    ...
    def push_source_status(self, delta: dict) -> None:
        ...
        non-owner or app-token clients. Fire-and-forget: clients retain the
        last local value and refresh explicitly when they need a new read.
        """
    ...
    def refresh_slot_source_status(self, key: str) -> None:
        """Republish this slot's cached PR/MR status after a turn boundary.

        Turn completion is an automatic lifecycle event and must never dispatch
        credential-backed provider commands. Explicit user refreshes own all
        provider reads; this hook only republishes the in-memory cache so an
        owner window can render whatever status is already known.
        """
        if not self._owner_ws_clients:
            return
        try:
            # Keep the call scoped to the finished slot for readability and to
            # retain the existing best-effort boundary. ``push_slots_update``
            # serializes all slots from the local cache and performs no provider
            # I/O, so the argument is intentionally not used for dispatch.
            if self.source_link_urls_for_slot(key):
                self.push_slots_update()
        except Exception:
            logger.debug("turn-boundary cached source status publish failed", exc_info=True)
