...
Connect and the explicit refresh-token action respond with the minted dashboard
**token** so the browser can set the embedded iframe's first-party cookie. Those
responses are the only places a token crosses this boundary; it is never logged,
and it never appears in list/status.
"""
...
def _status_for(state: "DashboardState", instance_id: str) -> dict:
    ...
    an explicit Connect/Retry, report an ``error`` state carrying that reason.
    A dropped live tunnel is reported as ``disconnected`` with its diagnostic
    text preserved, so the UI requires an explicit Retry.
    """
...
async def api_instances_list(request: web.Request) -> web.Response:
    """GET /api/instances — read local registry and in-memory status only.

    Listing never probes SSH, mints/refreshes a token, or diagnoses a remote
    host. Remote I/O starts only from an explicit owner action.
    """
...
async def api_instances_status(request: web.Request) -> web.Response:
    """GET /api/instances/{id}/status — local status snapshot for one instance.

    The default response is cache-only. ``?diagnose=1`` is the explicit
    owner-requested network probe.
    """
...
async def api_instances_connect(request: web.Request) -> web.Response:
    ...
    On success returns the live status plus the minted ``token`` to the owner
    (never logged, never in list/status). The separate refresh-token endpoint
    is also explicit and owner-only.
    """
    ...
    if status.state.value == "connected":
        ...
        # (a failed explicit token refresh, or a remote `kirocrew restart` that
        ...
        # and the browser-side Retry action can't help. Probe over the live tunnel (no
...
async def api_instances_refresh_token(request: web.Request) -> web.Response:
    ...
    embedded iframe with a valid credential after an explicit Refresh Token
    action. There is no TTL timer or automatic auth-expiry mint. Like
    ``connect``, this is the only other place a token crosses this boundary; it
    is never logged and never appears in list/status.
    """
