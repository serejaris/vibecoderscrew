...
def _register_instances_hooks(app: web.Application, state: DashboardState, port: int) -> None:
    ...
    (default off). Startup only constructs local control-plane objects. It never
    opens SSH tunnels or mints remote tokens from persisted ``was_connected``
    hints; the dashboard keeps those hints as sticky tabs and requires an
    explicit Connect/Retry action for every remote egress.
    """
    ...
    async def _instances_startup(app_: web.Application) -> None:
        ...
        # Do not schedule any reconnect/re-mint task here. Persisted
        # ``was_connected`` is display state only; the first SSH operation must
        # come from an explicit Connect/Retry request after startup.
...
async def start_dashboard(
    ...
    # Readiness is intentionally lazy. Constructing the service only latches
    # local state; the first Kiro/AWS probe is reserved for an explicit
    # readiness action (or a provider turn). A boot warm-up would spawn
    # kiro-cli while the dashboard is merely mounting and can open the
    # provider's browser login when the user is signed out.
    state.load_folders()
    state.load_tags()
    app["port"] = port
    ...
    async def _codex_login_shutdown(app_: web.Application) -> None:
        service = app_.get("codex_login_service")
        if service is not None:
            await service.close()

    app.on_cleanup.append(_codex_login_shutdown)
    ...
    # MCP discovery is explicit-action only.  The dashboard can mount with a
    # cached/config-only server list; probing a stdio or HTTP server belongs to
    # the authenticated Probe/Refresh action and must never run at boot.
...
async def start_api_server(
    ...
    state.kiro_prerequisite_service = app["kiro_prerequisite_service"]
    # Keep startup side-effect free: a readiness probe is an explicit action,
    # never a dashboard/API-server boot task. This prevents a signed-out
    # kiro-cli from opening its browser login while the server is mounting.
    state.load_folders()
    state.load_tags()
    app["port"] = port
