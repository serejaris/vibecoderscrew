...
def _finish_queue_cycle(state: DashboardState, slot: _ChatSlot) -> None:
    ...
    # Republish any cached source status for owner windows. Provider reads are
    # reserved for explicit user refresh actions; turn completion is automatic
    # and must stay provider/network-free.
            ...
            if event.kind == EVENT_TEXT_CHUNK:
                ...
                if _u.input_tokens or _u.output_tokens or _u.credits:
                    ...
                    await persist_token_record_async(
                        ...
                        # given, so compatibility consumers and the histogram
                        # agree when they inspect one in-memory row. acp reports
                        # 0 here.
