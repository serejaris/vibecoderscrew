...
async def _bg_mcp_probe() -> None:
    """Run an MCP probe and refresh the process cache.

    This helper is intentionally not scheduled by gateway/dashboard startup.
    Callers must be explicit user actions (for example the Probe/Refresh API).
    """
...
async def api_mcp_servers(request: web.Request) -> web.Response:
    ...
    and ``includeMcpJson`` are ignored by kiro-cli in ACP mode.  This is a
    cache/config-only read: it never starts a probe or MCP process.  Use the
    explicit ``POST /api/mcp/probe`` action when live discovery is wanted.
    """
...
async def api_mcp_probe_cached(request: web.Request) -> web.Response:
    """GET /api/mcp/probe — return the last cached probe results.

    A stale or empty cache is a valid result.  Live process/network discovery
    belongs to the explicit ``POST /api/mcp/probe`` action.
    """
