                                ...
                                for dp in data.get("data_points", []) or []:
                                    ...
                                    if name == _STARTUP_METRIC and is_hist:
                                        ...
                                        for ak in _OTHER_SPLIT_ATTRS:
                                            ...
                                            other_split.setdefault(name, {}).setdefault(
                                                sig, _Hist()
                                            ).add(dp)
                                    elif "value" in dp:
...
async def api_telemetry_startup(request: web.Request) -> web.Response:
    ...
    ``context`` is sourced from legacy per-turn token rows, NOT from the OTEL
    ...
    reported here because "how full is the window" belongs next to the other
    per-turn health signals. Existing manually supplied rows remain readable;
    the source-only runtime does not append new rows.
    """
