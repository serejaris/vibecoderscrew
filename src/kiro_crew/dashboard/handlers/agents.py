...
def _static_model_rows(provider: str) -> list[dict[str, Any]]:
    """Return a provider-local model catalog without starting a provider.

    UI mounts use this cache/static path. It intentionally contains no binary,
    auth, network, or provider construction work; a live entitlement refresh is
    available only through the explicit ``/api/models?refresh=1`` action.
    """
    if provider == "codex":
        return [{"model_name": "auto", "display_name": "Auto", "description": ""}]
    if provider not in {"acp", "codex"}:
        return []

    rows: list[dict[str, Any]] = []
    seen: set[str] = set()
    for entry in model_registry.display_list("acp"):
        canonical = str(entry.get("model_name", ""))
        model_name = model_registry.to_acp_id(canonical) if canonical != "auto" else "auto"
        if not model_name:
            model_name = "auto"
        ...
        window = model_registry.model_window(model_name)
        if window:
            row["context_window_tokens"] = window
        rows.append(row)
    return rows or [{"model_name": "auto", "display_name": "Auto", "description": ""}]


async def api_models(request: web.Request) -> web.Response:
    """GET /api/models — static catalog, or explicit live provider refresh."""
    cfg = KiroCrewConfig.load()
    if request.query.get("refresh") != "1":
        return web.json_response(_static_model_rows(cfg.agent.provider))
    ...
    if cfg.agent.provider not in {"acp", "codex"}:
        return web.json_response(
            {
    ...
    # Explicit refreshes still fail closed before resolving/spawning kiro-cli.
    # The live command can auto-open an interactive browser login when
    # unauthenticated, so readiness is checked only after the explicit marker.
