...
# An explicit usage refresh may perform two bounded whoami probes, a direct API
# request, and (when needed) a 60s text scrape. Keep the request itself bounded
# so a wedged provider cannot hold the dashboard connection forever.
_USAGE_REFRESH_TIMEOUT_SECS = 120.0
...
async def _fetch_usage_bg() -> None:
    """Fetch usage and update the cache.

    The caller may run this as an explicit bounded request action; the helper
    name remains for compatibility with existing tests and imports.
    """
...
async def api_sessions_usage(request: web.Request) -> web.Response:
    """GET /api/sessions/usage — cached usage, refreshed only explicitly.

    The default read is deliberately cache-only. ``kiro-cli /usage`` can open
    the provider's browser login when signed out, so a mount, status poll, or
    timer must never schedule it. Callers that have an explicit user action may
    pass ``?refresh=1``; that path performs the readiness check and waits for a
    single bounded refresh, returning the resulting cache (or an explicit
    timeout/error response) in the same request.
    """
    ...
    # Explicit refresh only. There is no TTL/timer trigger: the cache remains
    # stable until the user asks this endpoint to refresh it. Awaiting here is
    # deliberate — callers receive the fresh result or a bounded error without
    # needing a follow-up timer/poll that could re-enter the provider.
    try:
        await asyncio.wait_for(_fetch_usage_bg(), timeout=_USAGE_REFRESH_TIMEOUT_SECS)
    except asyncio.CancelledError:
        raise
    except asyncio.TimeoutError:
        logger.warning("Explicit Kiro usage refresh timed out")
        return web.json_response(
            {
                "usage": _usage_cache,
                "error": "Kiro usage refresh timed out",
                "code": "usage_refresh_timeout",
            },
            status=504,
        )
    except Exception:
        logger.warning("Explicit Kiro usage refresh failed", exc_info=True)
        return web.json_response(
            {
                "usage": _usage_cache,
                "error": "Kiro usage refresh failed",
