...
Commands (``kirocrew`` remains a compatibility alias):
    vibecoderscrew chat -m "message"    Send a single message
    vibecoderscrew chat                 Interactive chat mode
    vibecoderscrew gateway              Start the Vibecoders Crew server (dashboard + Slack)
    vibecoderscrew gateway --seed NAME  Populate $KIROCREW_HOME from fixture NAME, then start the gateway
    vibecoderscrew status               Show runtime stats
    vibecoderscrew run TASK.md          Run an autonomous task from a spec file
    vibecoderscrew update               Update Vibecoders Crew via git fetch + rebuild
    vibecoderscrew cron list|add|remove Manage scheduled jobs
    vibecoderscrew spawn run "task"     Spawn a background subagent
    vibecoderscrew spawn list           List subagents
    vibecoderscrew learn add|list|remove Save and manage learned corrections
    vibecoderscrew setup                Interactive credential setup
    vibecoderscrew doctor               Verify setup
"""
...
from kiro_crew.env import augmented_path, git_build_info
...
def _ensure_node(proj_dir: str = "", *, allow_install: bool = False) -> bool:
    """Check for a usable Node.js runtime without changing the host by default.

    Gateway/server startup must be deterministic and offline: an absent or old
    Node binary is a setup error, not permission to run ``curl``/mise/nvm and
    mutate the user's toolchain.  The source-tree installer remains available
    for an explicit operator action (currently ``kirocrew update``) by passing
    ``allow_install=True``.
    """
    if _node_ok():
        return True
    ...
    if not allow_install:
        _print_node_setup_hint(proj_dir)
        return False
...
def _print_node_setup_hint(proj_dir: str = "") -> None:
    """Print an actionable, user-authorized Node setup instruction."""
    root = Path(proj_dir) if proj_dir else Path.cwd()
    script = root / "ensure-node.sh"
    if not script.is_file():
        script = Path(__file__).resolve().parent.parent.parent / "ensure-node.sh"
    print(
        "Node.js >= 16 is required, but no usable Node.js executable was found.\n"
        "Install Node.js with your system/package manager, then retry.\n"
        f"For this source checkout, an explicit setup action is: bash {script}",
        file=sys.stderr,
    )
...
def _node_ok() -> bool:
    ...
    Finder/launchd starts the packaged Electron app with a minimal PATH. Use
    the shared environment augmentation so Homebrew, nvm/fnm, mise, and the
    running interpreter's bin directory remain discoverable in that context.
    """
...
def _resolve_gateway_args(args: argparse.Namespace) -> dict:
    ...
    open_browser = bool(getattr(args, "open_browser", False)) and not no_open
...
""",
    ...
    gw_parser = sub.add_parser(
        "gateway", help="Start the Vibecoders Crew server (dashboard + Slack)"
    )
    ...
    gw_parser.add_argument(
        "--no-open",
        action="store_true",
        help="Compatibility flag: suppress dashboard browser launch on startup",
    )
    gw_parser.add_argument(
        "--open",
        dest="open_browser",
        action="store_true",
        help="Explicitly open the dashboard URL in the default browser on startup",
    )
    gw_parser.add_argument(
        "--port",
        metavar="PORT",
...
""",
    ...
    snap_parser = sub.add_parser(
        "snapshot", help="Create a portable backup of Vibecoders Crew state"
    )
