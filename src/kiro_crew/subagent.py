...
class SubagentManager:
            ...
            try:
                ...
                out.append(
                    (
                        ...
                        conv_key,
                        sid,
                        # Provider labels became mandatory once Codex became the
                        # canonical backend. A legacy run without one cannot be
                        # safely classified as ACP, so keep it explicitly
                        # non-resumable instead of falling through to ACP.
                        str(state.get("provider") or "unknown"),
    ...
    def release_conversation(self, conv_id: str) -> tuple[bool, str]:
        ...
        # Missing labels must not be interpreted as ACP: the canonical
        # provider is Codex and native session files are incompatible.
        provider_label = self._sessions.conversation_provider(conv_key) or "unknown"
    ...
    async def _run_inner(self, info: SubagentInfo, session_key: str) -> None:
        ...
        provider_label = self._provider_label(client, is_cc=is_cc)
    ...
    @staticmethod
    def _provider_label(provider: object, *, is_cc: bool = False) -> str:
        """Return an explicit persistence label for a subagent provider.

        Provider identity is part of the session-file contract. In particular,
        a missing/unknown identity must remain ``unknown``; treating every
        non-Claude implementation as ACP would let a Codex run clean up or
        resume the wrong native transcript.
        """
        if is_cc:
            return "claude_code"
        try:
            ...
            explicit = ""
        if isinstance(explicit, str) and explicit in {"acp", "codex", "claude_code"}:
            return explicit
        # Session sharing uses this adapter directly. It is an ACP session
        # created on the parent's Kiro runtime and therefore has no native
        # provider_id property of its own.
        if isinstance(provider, AcpSessionProvider):
            return "acp"
        # Keep the concrete ACP adapter explicit for test doubles and legacy
        # callers whose provider_id property is absent or malformed.
        try:
