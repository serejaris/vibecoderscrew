...
def detect_provider_switch(session_map: "SessionMap", session_key: str, new_provider: str) -> bool:
    ...
    """
    stored_provider = session_map.get_provider(session_key)
    if stored_provider not in {"acp", "codex", "claude_code"}:
        # Legacy maps omitted provider entirely. Treat that SID as
        # non-resumable; inferring ACP would let the Codex default reuse an
        # incompatible native transcript.
        return False
...
class SessionManager:
    ...
    def configured_provider(self) -> str:
        ...
        return str(getattr(self._cfg.agent, "provider", "codex") or "codex")
    ...
    def _bg_provider_is_kiro(self) -> bool:
        ...
        """
        try:
            # A missing provider on a fresh config follows the canonical
            # Codex default. An explicit ``acp`` value still selects the
            # multiplexed Kiro runtime below.
            prov = getattr(self._cfg.agent, "provider", "codex") or "codex"
        except Exception:
            prov = "codex"
        return prov == "acp"
