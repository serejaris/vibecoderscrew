...
Credentials are resolved by the ``aws`` CLI's own provider chain — VibecodersCrew
...
class CloudActionDenied(Exception):
    ...
    ``deniedCommands`` block the ``aws`` CLI verbs and the ``vibecoderscrew cloud``
...
def assert_human_action(action: str) -> None:
    ...
    (and MCP tools key off it). A human running ``vibecoderscrew cloud …`` in their own
    ...
    if _in_agent_session():
        raise CloudActionDenied(
            ...
            "from an agent session — run `vibecoderscrew cloud …` yourself in a terminal."
        )
...
def assert_chokepoint_allowed(args: list[str]) -> None:
    ...
    if _in_agent_session() and (tuple(args[:2]) not in _AGENT_READ_ALLOWLIST):
        ...
        raise CloudActionDenied(
            ...
            "`vibecoderscrew cloud …` yourself in a terminal."
        )
        ...
        try:
            ...
            return (
                ...
                (
                    ...
                    "or install the AWS CLI from its official guide, then retry"
                ),
            )
        try:
    ...
    *,
    ...
    if rc != 0:
        ...
        if missing:
            ...
            hint = " — add the missing permission (see `vibecoderscrew cloud iam-policy`) and retry"
        raise AWSError(
