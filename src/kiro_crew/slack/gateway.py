...
async def _persist_turn_row(
    ...
    *,
    ...
    """Build one compatibility usage row for a background dispatch surface.
    ...
    compatibility-row failure is logged at debug and never propagates into the
    background loop, since usage accounting must not abort a live turn.
    ...
    try:
        ...
        logger.debug("usage compatibility row (%s) failed", surface, exc_info=True)
...
class GatewayOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        # Compatibility slot for an explicit update task created by older
        # integrations. Current boot keeps update checks user-triggered.
    ...
    def _check_missing_deps(self) -> list[str]:
        """Return missing runtime dependencies and an actionable remedy.

        Gateway boot is a read-only lifecycle phase.  In particular, a missing
        optional package must not turn a status/dashboard launch into an
        implicit ``pip install`` (which can reach the network and mutate the
        active environment).  Installation remains an explicit operator action.
        """
    ...
    def _init_services(self) -> None:
        ...
        # Dependency readiness is check-only during boot.  Provider version and
        # authentication checks belong to the explicit prerequisite action.
        try:
            self._check_missing_deps()
        except Exception:
            logger.error("Dependency readiness check failed", exc_info=True)
    ...
    async def _init_cron(self) -> None:
        ...
        async def _cron_callback(job: CronJob) -> str | None:
                    ...
                    try:
                        ...
                        try:
                            ...
                            await persist_token_record_async(
                                ...
                                provider_last_turn_usage(client),
                                provider=(
                                    self._cfg.agent.provider if hasattr(self, "_cfg") else "codex"
                                ),
                                surface="cron",
            ...
            try:
                ...
                _provider = self._cfg.agent.provider if hasattr(self, "_cfg") else "codex"
                # Off-loop: build_message embeds the episodic query.
                full_message, _ = await run_in_embed_pool(
                    self.ctx_builder.build_message,
                    msg,
        ...
        async def _heartbeat_task(task_text: str, deliver: str) -> str | None:
            ...
            try:
                ...
                await _persist_turn_row(
                    client,
                    session_key,
                    provider=(self._cfg.agent.provider if hasattr(self, "_cfg") else "codex"),
                    surface="heartbeat",
    ...
    async def _fire_dashboard_nudge(self, loop: NudgeLoop) -> bool:
        ...
        if slot is None:
            ...
            # thread-safe.
            slot = await rehydrate_slot_from_history_async(self.dashboard_state, loop.slot_key)
            if slot is None:
                logger.warning(
    ...
    async def _shutdown(self) -> None:
        ...
        # Keep compatibility with older orchestrator instances that may have
        # installed an explicit update task.  Current boot never creates one;
        # explicit update requests own their task lifecycle.
    ...
    def _browser_open_requested(self) -> bool:
        """Return whether dashboard startup may launch the user's browser."""
        return not self._no_open and (self._open_browser or self._cfg.dashboard.auto_open_browser)
    ...
    async def run(self) -> None:
        ...
        # ── Signal handlers ──
        # Install these before any long-lived background service is armed so a
        # stalled optional service can never make boot uninterruptible.
        loop = asyncio.get_running_loop()
        ...
        # Optional update/MCP work is explicit-action only.  In particular, do
        # not call ``_check_for_updates`` or ``_bg_mcp_probe`` here: both can
        # reach the network or spawn child processes before a user asks for
        # either operation.  ``/api/update/check`` and the MCP Probe action own
        # those effects and keep their results in the existing caches.

        # ── Print URLs (provider startup is explicit-action only) ──
        async def _announce_dashboard() -> None:
            if not self._no_dashboard:
                ...
                if not self._browser_open_requested():
                    pass  # browser launch is opt-in via --open or explicit config
                elif _skip_open:
        ...
        # URL announcement/browser opening is independent of provider auth. Do
        # not call ``sessions.start_pool`` here: that method creates an ACP/
        # Codex background provider before the first user turn.
        asyncio.create_task(_announce_dashboard())
