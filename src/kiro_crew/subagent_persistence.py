        ...
        try:
            ...
            if ts.get("died", 0) < cutoff:
                ...
                try:
                    ...
                    # Old tombstones may omit provider. Preserve that
                    # uncertainty: ACP cleanup is only safe when the label is
                    # explicit, especially now that Codex is canonical.
                    provider = ts.get("provider") or (
                        state.get("provider", "unknown") if state else "unknown"
                    )
...
def _cleanup_session_files_sync(
    session_id: str, provider: str = "unknown", *, cwd: str = ""
) -> None:
