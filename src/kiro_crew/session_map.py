...
_RESUMABLE_PROVIDERS = frozenset({"acp", "codex", "claude_code"})
...
class SessionMap:
    ...
    def get(self, key: str) -> str | None:
        ...
        if provider in {"claude_code", "codex"}:
            return sid
        if provider != "acp":
            # Pre-provider session maps cannot identify whether the native SID
            # belongs to ACP or Codex. Reusing it would silently resume an ACP
            # transcript under the Codex default, so legacy entries fail closed
            # until a fresh turn writes an explicit provider label.
            logger.warning(
                "Session %s has no explicit provider; refusing native resume for %s",
                sid,
                key,
            )
            return None
    ...
    def get_provider(self, key: str) -> str:
        """Return the explicit stored provider for *key*, or ``''`` if absent."""
        entry = self._data.get(canonical_key(key))
        if not entry:
            return ""
    ...
    def prune(self) -> int:
        ...
        stale = [
            ...
            if entry.get("provider") not in _RESUMABLE_PROVIDERS
            and (
