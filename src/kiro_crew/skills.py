...
# by file recency and summarized top-down; the tail is discoverable via
# `skill_search`. No persistent activity log participates in this ranking.
...
# ranking so a freshly-added skill still surfaces.
...
# file metadata at lifecycle-run time).
...
class SkillsLoader:
    ...
    def __init__(
        ...
        # Usage ranking is intentionally stateless.  Skill loading must never
        # create or update a persistent activity log in the data home.
    ...
    def _auto_activity(self, key: str, path_str: str, meta: dict) -> tuple[int, float]:
        ...
        Persistent usage is disabled, so lifecycle recency comes from the
        created_at frontmatter or the file mtime. A fresh skill is not treated
        as ancient merely because it has no activity log.
        """
        hits = 0
        last_seen = 0.0
    ...
    def get_triggered_skills(self, text: str) -> list[str]:
        ...
        # Keep the compatibility hook for callers that inspect the triggered
        # set. It intentionally records no durable activity signal.
    ...
    def get_context(self, budget: int | None = None, only: list[str] | None = None) -> str:
        ...
        them). The remaining on-demand skills are ranked by file recency and
        summarized top-down until *budget* chars are consumed; the long tail is left
        ...
        ON) switches to the bounded, recency-ranked top-K described above.
    ...
    def _record_use(self, key: str) -> None:
        """Compatibility hook; usage activity is intentionally not recorded."""
    ...
    def _rank_key(self, s: dict) -> tuple[float, float]:
        """Sort key for on-demand skills based on effective file recency."""
    ...
    def search_skills(self, query: str, limit: int = 20) -> list[dict]:
        ...
        for s in self.list_skills():
            ...
            if total <= 0:
                continue
            # Persistent usage ranking is disabled in the source-only edition.
            usage = 0.0
