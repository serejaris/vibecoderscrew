...
async def api_suggestions(request: web.Request) -> web.Response:
    """GET /api/suggestions — return local suggestions by default.

    Query params:
        force=1  — explicitly generate fresh provider-backed suggestions
    """
    ...
    if force:
        try:
            ...
            pass
    # The ordinary GET is intentionally cache/local-only. In particular, do
    # not call ``maybe_refresh`` here: WelcomeView mounts this endpoint and a
    # stale cache must never turn a status/UI read into a provider prompt.
