...
"""Feature Tips — local catalog discovery with weighted-random selection.
...
# Refresh the local catalog-derived pool every 6 hours.
_REFRESH_INTERVAL_SECS = 6 * 60 * 60
# Keep the legacy parser bound for old persisted state; no model generation
# path calls it in the source-only release.
...
class TipsState:
    ...
    # Permanent ack by catalog doc: older generated tips could invent ids on
    # every refresh, so id-only matching let a dismissed feature resurface.
    # Doc remains the stable feature identity for legacy state.
    dismissed_docs: list[str] = field(default_factory=list)
    snoozed: dict[str, float] = field(default_factory=dict)  # tip_id -> snooze_ts
    # Snooze by stable doc identity for compatibility with older generated
    # state; current catalog ids are already stable.
    ...
    # and a refresh may replace st.tips before the user dismisses, so a stale id
    # can still resolve back to its stable doc identity.
...
def _save_state(st: TipsState) -> None:
    ...
    atomic_write(
        ...
        + "\n",
        # Owner-only: legacy tip state may contain user-authored feedback and
        # older generated text. Keep it private on shared machines; current
        # tips are catalog-derived and do not collect activity context.
        mode=0o600,
    )
...
@dataclass
class TipsCache:
    """In-memory local-catalog tips cache with periodic refresh."""
    ...
    try:
        ...
        for e in entries_raw:
            ...
            if feature and summary and doc:
                ...
                entries.append(
                    CatalogEntry(
                        feature=feature, summary=truncate_summary(summary), doc=doc, mtime=mtime
                    )
                )
...
def _select_tip(
    ...
    # Legacy generated tips carry the catalog doc in "doc" but may invent
...
# The projection remains for compatibility with older persisted tip state.
_TIP_ALLOWED_FIELDS = ("id", "feature", "title", "body", "why", "doc", "cta_prompt")


def _parse_tips(text: str) -> list[dict]:  # type: ignore[type-arg]
    """Parse a legacy generated-tip payload into validated tip dicts."""
    text = text.strip()
    if text.startswith("```"):
        lines = text.split("\n")
    ...
    try:
        ...
        if isinstance(result, list):
            ...
            for t in result:
                ...
                # Unknown nested values would bypass string-only redaction and
                # reach legacy persistence + the dashboard.
...
# Field length limits for legacy generated tip dicts.
...
# hand-authored actions; legacy generated tips never get one, so no invented
# route can reach the client.
...
def _fallback_tips(catalog: list[CatalogEntry], st: TipsState) -> list[dict]:  # type: ignore[type-arg]
    """Build local catalog tips without reading activity or memory context."""
    tips = []
...
def _is_eligible(
    ...
    if tid in st.dismissed:
        return False
    # Doc-level dismissal: stable across legacy refreshes that may change ids
    doc = tip.get("doc", "")
...
async def generate_tips(state: DashboardState) -> list[dict]:  # type: ignore[type-arg]
    """Return catalog-derived tips without opening a model or background session.

    The async API remains for callers that schedule periodic refreshes. The
    source-only release deliberately keeps selection local and deterministic in
    its data sources, so showing a tip cannot trigger network-capable model
    work or collect recent activity.
    """
    cache = await get_tips_cache(state)
    return _fallback_tips(cache.catalog, cache.state)


async def refresh_tips(state: DashboardState, cache: TipsCache) -> None:
    """Refresh local catalog tips in a task outside the cache lock.

    The task is retained as a cheap compatibility scheduler; it performs no
    model call and reads no user activity.
    """
    try:
...
async def api_tips_next(request: web.Request) -> web.Response:
    ...
    # the lock is fast (list filters + one small file write); refresh work is
    ...
    async with cache._lock:
        ...
        # Legacy generated tips in state augment them. Dedupe by id — curated wins.
...
async def api_tips_feedback(request: web.Request) -> web.Response:
    ...
    async with cache._lock:
        if action == "shown":
            ...
            # Also record the catalog doc (stable feature identity): legacy
            # regeneration could invent fresh ids, so id-only dismissal would
            # let the same feature resurface.
            if tip_id:
                doc = ""
                if isinstance(st.offered, dict) and st.offered.get("id") == tip_id:
