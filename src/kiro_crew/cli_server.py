...
def _update() -> None:
    ...
    if not _ensure_node(proj, allow_install=True):
        print("  ❌ Node.js setup failed; update cannot build the dashboard.", file=sys.stderr)
        sys.exit(1)
...
async def _gateway(
    *,
    ...
    # Startup is a read-only prerequisite check.  In particular, do not invoke
    # ensure-node.sh or a version manager here: those scripts may install mise,
    # nvm, or Node over the network.  A missing runtime is an actionable setup
    # failure and must be repaired by an explicit operator command.
    from kiro_crew.cli import _ensure_node, _node_ok  # circular import: cli -> cli_server -> cli

    if not _node_ok():
        if not _ensure_node():
            raise RuntimeError(
                "Node.js >= 16 is required to start the gateway. "
                "Install it explicitly, then retry `kirocrew gateway`."
            )
