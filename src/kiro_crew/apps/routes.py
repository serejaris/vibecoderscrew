...
async def handle_registry(request: web.Request) -> web.Response:
    ...
    Plain GETs are deliberately local/cache-only.  ``?refresh=1`` is the
    explicit Discover/Refresh marker that permits remote registry and manifest
    fetches for this request.
    """
    refresh = request.query.get("refresh", "").strip().lower() in {"1", "true", "yes"}
    apps = await list_registry(refresh=refresh)
    return web.json_response(
        {
            "apps": apps,
...
async def handle_blob_proxy(request: web.Request) -> web.Response:
    ...
    # A browser image request is part of a passive page mount, so it must not
    # turn a missing cache entry into a git clone.  The list route adds the
    # explicit ``refresh=1`` marker only after the user requests Discover or
    # Refresh; installs remain explicit clone actions in their own endpoint.
    remote_allowed = request.query.get("refresh", "").strip().lower() in {"1", "true", "yes"}
    if not cache_path.is_file() and not remote_allowed:
        return web.json_response(
            {
                "error": "blob is not cached; use an explicit registry refresh",
                "code": "blob_not_cached",
            },
            status=404,
        )
