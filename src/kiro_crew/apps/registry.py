...
def _read_manifest_cache(
    ...
    *,
    ...
    """Read cached app.json for a registry app.

    The default App Store browse path is intentionally cache-only.  It may use
    a stale cache entry when the user has not explicitly requested a refresh;
    stale data is safer than turning a passive page load into a git clone.
    """
    ...
    try:
        ...
        if age > _MANIFEST_CACHE_TTL and not ignore_ttl:
            return None  # stale
...
async def _resolve_manifest(
    ...
    *,
    ...
    cached = await asyncio.to_thread(
        ...
        name,
        ignore_ttl=not allow_remote,
    )
    if cached:
        return _merge_manifest(entry, cached, allow_remote=allow_remote)

    # A normal GET /api/apps/registry is a static/cache read.  Remote manifest
    # fetches require an explicit refresh/discover action from the caller.
    if not allow_remote:
        return entry
    ...
    if manifest:
        ...
        return _merge_manifest(entry, manifest, allow_remote=allow_remote)
...
def _merge_manifest(
    ...
    *,
    ...
    def _blob_url(path: str) -> str:
        url = f"/api/apps/blob?repo={repo}&path={path}"
        return f"{url}&refresh=1" if allow_remote else url
...
async def _load_external_registries(*, allow_remote: bool = False) -> list[dict[str, Any]]:
    ...
    source for UI grouping.  ``allow_remote`` is reserved for an explicit
    Refresh/Discover action; the default only reads the local cache (including
    stale entries) and never invokes git.
    """
    from kiro_crew.config.loader import (
    ...
    async def _load_one(reg) -> list[dict[str, Any]]:
        ...
        if not allow_remote:
            # Passive browsing is cache/static-only.  Read stale cache too so
            # an expired index remains visible while offline rather than
            # disappearing.
        ...
        # Fetch from repo only after an explicit Refresh/Discover marker.  A
        # refresh deliberately bypasses even a fresh cache so the user action
        # means "check the remote now".
        entries = await _fetch_and_cache_external_registry(reg)
        if entries is not None:
            return entries

        # Fall back to stale cache (stale > missing) after an explicit refresh
        # fails, preserving the last known catalog.
        stale = await asyncio.to_thread(
            _read_external_registry_cache,
            name,
            ignore_ttl=True,
...
async def refresh_registries(repo: str | None = None) -> dict[str, Any]:
    ...
    *repo*), refetches its index and — only on a successful fetch — overwrites
    ...
    apps = await list_registry(refresh=True)
...
async def list_registry(*, refresh: bool = False) -> list[dict[str, Any]]:
    """Return all registry apps with display info and install status.

    1. Load minimal registry JSON (name, repo, branch)
    2. Load external registries from the local cache (or fetch them only when
       ``refresh=True``)
    3. Read each app's cached app.json; remote fetches require ``refresh=True``
    4. Run detectInstalled commands only for an explicit refresh
    ...
    external_entries = await _load_external_registries(allow_remote=refresh)
    ...
    resolved = await asyncio.gather(
        *[_resolve_manifest(e, allow_remote=refresh) for e in entries],
        return_exceptions=True,
    )
    entries = [r if isinstance(r, dict) else entries[i] for i, r in enumerate(resolved)]

    # Detection executes an app-provided shell command.  Keep passive browse
    # static/cache-only as well; explicit refresh is the user action that opts
    # into this probe.
