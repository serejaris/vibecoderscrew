...
def _effective_window(window_tokens: int | None) -> int:
    ...
    ``provider=codex`` + ``model="auto"`` on a 1M-window model. Treating an
...
def resolve_model_window(model: str | None) -> int | None:
    ...
    default deployment runs ``provider=codex`` + ``model="auto"`` on a
    1M-window model, so we must NOT scale down here — ``None`` keeps the
    reference.
    ...
    Opus 4.8 is 200K regardless of whether it is reached via Codex, kiro-cli,
    or ``claude_code`` — membership and window are provider-independent (see
    ...
    provider, which silently no-op'd the whole feature on the default path.)
    """
